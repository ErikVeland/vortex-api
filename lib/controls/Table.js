...
class SuperTable extends ComponentEx_1.ComponentEx {
    constructor(props) {
        ...
        this.renderBody = () => {
            ...
            if (groupedRows !== undefined) {
                return (React.createElement(MyTable_1.TBody, null, groupedRows.map(group => {
                    ...
                    return [(React.createElement(GroupingRow_1.default, { t: t, key: group.id || '__empty', groupId: group.id, groupName: group.name, expanded: expanded, count: group.count, width: this.mVisibleAttributes.length + 1, onToggle: this.toggleGroup, onExpandAll: this.expandAll, onCollapseAll: this.collapseAll })),
        ...
        this.scrollTo = (id, mayRetry) => {
            try {
                ...
                if (mayRetry !== false) {
                    ...
                    (0, log_1.log)('warn', 'failed to scroll to item', { id, tableId: this.props.tableId, error: err.message });
                }
            }
        };
        ...
        this.scrollToItem = (item, smooth, iterations = 3) => {
            ...
            if ((targetPos !== undefined)
                ...
                if (smooth) {
                    (0, smoothScroll_1.default)(this.mScrollRef, targetPos, SuperTable.SCROLL_DURATION)
                        .then((cont) => cont && (iterations > 0)
    ...
    renderFooter() {
        ...
        if (selected.length < 2) {
            const modifierKey = (0, platform_1.isMacOS)() ? 'cmd' : 'ctrl';
            return (React.createElement(Usage_1.default, { infoId: 'table-multiselect' }, t('Did you know? You can select multiple items using {{modifierKey}}+click or shift+click or '
                + 'select everything using {{modifierKey}}+a and then do things with all selected items at once.', { modifierKey })));
        }
    ...
    updateColumnWidth() {
        ...
        this.mProxyHeaderRef.childNodes.forEach((node, index) => {
            this.mVisibleHeaderRef.childNodes.item(index).style.minWidth =
                this.mVisibleHeaderRef.childNodes.item(index).style.maxWidth =
                    `${node.clientWidth}px`;
        });
    ...
    updateCalculatedValues(props, forceUpdateId) {
        ...
        return (0, promise_helpers_1.promiseMap)(Object.keys(data), (rowId) => {
            const delta = {};
            return (0, promise_helpers_1.promiseMap)(objects, (attribute) => {
            ...
            .then(() => (0, promise_helpers_1.promiseMap)(Object.keys(oldState.data), rowId => {
            if (data[rowId] === undefined) {
