...
function installExtensionDependencies(api, extPath) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            return Promise.resolve(bluebird_1.default.map(handler.dependencies, (depId) => {
...
function flattenNestedRoot(root) {
    return Promise.resolve(fs.readdirAsync(root)
        .then((entries) => bluebird_1.default.map(entries, (name) => fs.statAsync(path.join(root, name))
        .then(stat => ({ name, stat }))
        .catch(() => null)))
        ...
        if ((files.length === 0) && (dirs.length === 1)) {
            ...
            return fs.readdirAsync(inner)
                .then(innerEntries => bluebird_1.default.map(innerEntries, (innerName) => fs.renameAsync(path.join(inner, innerName), path.join(root, innerName))))
