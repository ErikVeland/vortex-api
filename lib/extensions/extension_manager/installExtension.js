...
function extractArchiveNativeSync(archivePath, destPath) {
    ...
    (0, log_1.log)('info', 'Starting native archive extraction', {
        archivePath,
        destPath,
        extension: ext,
        platform: process.platform,
    ...
    if (ext === '.zip') {
        ...
        if (isDarwin) {
            ...
            (0, log_1.log)('debug', 'Available ZIP extraction tools', {
                ditto: hasDitto,
                unzip: hasUnzip,
            ...
            if (!packaged7z && !hasDitto && !hasUnzip) {
                ...
                (0, log_1.log)('error', 'No ZIP extraction tools available on macOS', {
                    hasDitto,
                    hasUnzip,
                    hasPackaged7z: !!packaged7z,
        ...
        try {
            ...
            (0, log_1.log)('info', 'ZIP extraction completed', { archivePath, destPath });
            ...
            while (verifyAttempts < maxVerifyAttempts) {
                try {
                    ...
                    if (stats.isDirectory()) {
                        ...
                        if (files.length > 0) {
                            (0, log_1.log)('debug', 'Extraction verification successful', {
                                destPath,
                                fileCount: files.length,
            ...
            (0, log_1.log)('error', 'ZIP extraction failed', {
                archivePath,
                destPath,
                error: err.message,
                platform: process.platform
        ...
        cmds.push({ cmd: '7zz', args: ['x', '-y', archivePath, `-o${destPath}`] }, { cmd: '7z', args: ['x', '-y', archivePath, `-o${destPath}`] }, { cmd: 'unar', args: ['-force-overwrite', '-o', destPath, archivePath] });
        if (isDarwin) {
            const has7zz = hasCommandSync('7zz');
            const has7z = hasCommandSync('7z');
            const hasUnar = hasCommandSync('unar');
            // Log which tools are available
            (0, log_1.log)('debug', 'Available 7z extraction tools', {
                '7zz': has7zz,
                '7z': has7z,
                'unar': hasUnar,
                packaged7z: !!packaged7z
            ...
            if (!packaged7z && !has7zz && !has7z && !hasUnar) {
                ...
                (0, log_1.log)('error', 'No 7z extraction tools available on macOS', {
                    has7zz,
                    has7z,
                    hasUnar,
                    hasPackaged7z: !!packaged7z,
        ...
        try {
            const result = tryCommandsSync(cmds);
            (0, log_1.log)('info', '7z extraction completed', { archivePath, destPath });
            // Add a small delay and verification to ensure files are written
            const verifyStartTime = Date.now();
            let verifyAttempts = 0;
            const maxVerifyAttempts = 10;
            ...
            (0, log_1.log)('error', '7z extraction failed', {
                archivePath,
                destPath,
                error: err.message,
                platform: process.platform
        ...
        if (isDarwin) {
            ...
            // Log which tools are available
            (0, log_1.log)('debug', 'Available RAR extraction tools', {
                'unar': hasUnar,
                'unrar': hasUnrar,
                packaged7z: !!packaged7z
            });
            if (!packaged7z && !hasUnar && !hasUnrar) {
                ...
                (0, log_1.log)('error', 'No RAR extraction tools available on macOS', {
                    hasUnar,
                    hasUnrar,
                    hasPackaged7z: !!packaged7z,
                    systemPath: process.env.PATH
        ...
        try {
            const result = tryCommandsSync(cmds);
            (0, log_1.log)('info', 'RAR extraction completed', { archivePath, destPath });
            // Add a small delay and verification to ensure files are written
            const verifyStartTime = Date.now();
            let verifyAttempts = 0;
            const maxVerifyAttempts = 10;
            ...
            (0, log_1.log)('error', 'RAR extraction failed', {
                archivePath,
                destPath,
                error: err.message,
                platform: process.platform
    ...
    if (ext === '.tar' || ext === '.gz' || ext === '.bz2' || ext === '.xz' || archivePath.endsWith('.tar.gz') || archivePath.endsWith('.tgz')) {
        try {
            ...
            (0, log_1.log)('info', 'TAR extraction completed', { archivePath, destPath });
            // Add a small delay and verification to ensure files are written
            const verifyStartTime = Date.now();
            let verifyAttempts = 0;
            const maxVerifyAttempts = 10;
            ...
            (0, log_1.log)('error', 'TAR extraction failed', {
                archivePath,
                destPath,
                error: err.message,
                platform: process.platform
    ...
    (0, log_1.log)('error', 'Unsupported archive type detected', {
        archivePath,
        extension: ext,
        platform: process.platform
    });
