 ...
 */
function retryValidationOperation(operation_1) {
    return __awaiter(this, arguments, void 0, function* (operation, maxRetries = 5, baseDelayMs = 200, maxDelayMs = 5000, operationName = 'validation operation', shouldRetry) {
        let lastError;
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                (0, log_1.log)('debug', `Attempting ${operationName}`, { attempt, maxRetries });
                const result = yield operation();
                if (attempt > 1) {
                    (0, log_1.log)('info', `${operationName} succeeded on retry`, {
                        attempt,
                ...
                (0, log_1.log)('debug', `${operationName} failed on attempt ${attempt}`, {
                    attempt,
                    maxRetries,
                    error: error.message,
                    stack: error.stack,
                ...
                if (!shouldRetryFunc(error)) {
                    (0, log_1.log)('debug', `Not retrying ${operationName} - error not retryable`, {
                        error: error.message,
                        code: error.code
                    });
                    throw error;
                ...
                if (process.platform === 'darwin') {
                    ...
                    (0, log_1.log)('debug', `Adjusted delay for macOS APFS timing issues`, {
                        originalDelay: Math.round(delayWithJitter),
                        adjustedDelay: Math.round(platformAdjustedDelay),
                        platform: process.platform
                    });
                }
                (0, log_1.log)('debug', `Retrying ${operationName} in ${Math.round(platformAdjustedDelay)}ms`, {
                    attempt,
                ...
                yield new Promise(resolve => setTimeout(resolve, platformAdjustedDelay));
            }
        }
        (0, log_1.log)('warn', `${operationName} failed after ${maxRetries} attempts`, {
            ...
            return (callback) => {
                ...
                try {
                    ...
                    if (result && typeof result.then === 'function') {
                        result.catch((err) => {
                            (0, log_1.log)('warn', '⚠️ Extension once callback failed', { error: err.message });
                        });
                    }
                }
                catch (err) {
            ...
            return (callback) => {
                ...
                if (process.type !== 'renderer') {
                    try {
                        ...
                        (0, log_1.log)('warn', '⚠️ Extension onceMain callback failed', { error: err.message });
                    }
                }
            };
        }
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const vortexExt = {
                ...
                registerGameStub: (game, ext) => {
                    context.registerGameStub(game, ext);
                },
                registerGameInfoProvider: (id, priority, expireMS, keys, query) => {
                    context.registerGameInfoProvider(id, priority, expireMS, keys, query);
                },
                registerModType: (...args) => {
                    ...
                    try {
                        ...
                        (0, log_1.log)('warn', 'registerModType shim failed, invalid arguments', { error: err.message, args });
                    }
                },
                registerAction: (group, priority, icon, options, title, action, condition) => {
                ...
                registerAPI: (name, func, options) => {
                    context.registerAPI(name, func, options);
                },
                registerMainPage: (id, title, component, options) => {
                    context.registerMainPage(id, title, component, options);
                },
                registerFooter: (id, component) => {
                    context.registerFooter(id, component);
                },
                registerBanner: (id, component) => {
                    context.registerBanner(id, component);
                },
                registerOverlay: (id, component, props) => {
            ...
            return Promise.resolve(Promise.all(handler.dependencies.map((depId) => {
...
function flattenNestedRoot(root) {
    return Promise.resolve(fs.readdirAsync(root)
        .then((entries) => Promise.all(entries.map((name) => fs.statAsync(path.join(root, name))
        .then(stat => ({ name, stat }))
        .catch(() => null))))
        .then((items) => (items || []).filter((it) => it !== null))
        .then((items) => {
        ...
        if ((files.length === 0) && (dirs.length === 1)) {
            ...
            return fs.readdirAsync(inner)
                .then(innerEntries => Promise.all(innerEntries.map((innerName) => fs.renameAsync(path.join(inner, innerName), path.join(root, innerName)))))
 ...
 */
function hasExtensionFiles(extractionPath) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const files = yield fs.readdirAsync(extractionPath);
            ...
            for (const file of files) {
                try {
                    ...
                    if (stat.isDirectory()) {
                        const subFiles = yield fs.readdirAsync(filePath);
                        const hasSubExtensionFiles = subFiles.some(subFile => extensionIndicators.includes(subFile) || subFile.endsWith('.js'));
...
function waitForExtractionCompletion(extractionPath_1) {
    return __awaiter(this, arguments, void 0, function* (extractionPath, maxWaitMs = 15000) {
        ...
        let lastFileList = [];
        (0, log_1.log)('debug', 'Starting enhanced extraction completion monitoring', {
            extractionPath,
            maxWaitMs,
            platform: process.platform
        });
        ...
        while (Date.now() - startTime < effectiveMaxWaitMs) {
            try {
                ...
                const stats = yield fs.statAsync(extractionPath);
                if (!stats.isDirectory()) {
                    (0, log_1.log)('debug', 'Extraction directory not found, waiting...', { extractionPath });
                ...
                if (currentFileCount > 0) {
                    try {
                        ...
                        if (firstFile) {
                            ...
                            yield fs.statAsync(firstFilePath);
                        }
                    }
                    catch (accessErr) {
                        (0, log_1.log)('debug', 'Files listed but not yet accessible, continuing to wait', {
                            extractionPath,
                ...
                (0, log_1.log)('debug', 'Monitoring extraction progress', {
                    extractionPath,
                    currentFileCount,
                    lastFileCount,
                    stableCount,
                ...
                if (currentFileCount > 0) {
                    ...
                    const hasExtFiles = yield hasExtensionFiles(extractionPath);
                    if (hasExtFiles) {
                        (0, log_1.log)('debug', 'Extension files detected, extraction complete', {
                            extractionPath,
                    ...
                    for (const entry of files) {
                        try {
                            ...
                            const entryStat = yield fs.statAsync(entryPath);
                            if (entryStat.isDirectory()) {
                                const subFiles = (yield fs.readdirAsync(entryPath))
                                    .filter(f => !f.startsWith('.') && (f !== '__MACOSX'));
                                ...
                                if (presentSubKeys.length > 0) {
                                    ...
                                    for (const kf of presentSubKeys) {
                                        try {
                                            yield fs.statAsync(path.join(entryPath, kf));
                                        }
                                        catch (_a) {
                                    ...
                                    if (subAllAccessible) {
                                        (0, log_1.log)('debug', 'Key files found in subdirectory during extraction monitoring', {
                                            extractionPath,
                                            subdir: entry,
                                ...
                                if (subJs) {
                                    try {
                                        yield fs.statAsync(path.join(entryPath, subJs));
                                        (0, log_1.log)('debug', 'JS file accessible in subdirectory during extraction monitoring', {
                                            extractionPath,
                                            subdir: entry,
                                            sampleFile: subJs,
                                            waitedMs: Date.now() - startTime,
                    ...
                    if (currentFileCount === lastFileCount && filesEqual) {
                        ...
                        if (stableCount >= stabilityThreshold) {
                            (0, log_1.log)('debug', 'Extraction directory stable, extraction complete', {
                                extractionPath,
                                fileCount: currentFileCount,
                                waitedMs: Date.now() - startTime,
                    ...
                    (0, log_1.log)('debug', 'Files disappeared, resetting monitoring', { extractionPath });
                    hasFoundFiles = false;
                    stableCount = 0;
                    lastFileCount = 0;
                    lastFileList = [];
                ...
                if (err.code === 'ENOENT') {
                    ...
                    (0, log_1.log)('debug', 'Directory not found, continuing to wait', { extractionPath, error: err.code });
                    yield new Promise(resolve => setTimeout(resolve, checkInterval));
                    checkInterval = Math.min(maxCheckInterval, checkInterval * 2);
                }
                else {
                    // Other error, log and re-throw
                    (0, log_1.log)('error', 'Error during extraction monitoring', { extractionPath, error: err.message });
                    throw err;
                }
            }
        }
        ...
        try {
            const finalFiles = (yield fs.readdirAsync(extractionPath))
                .filter(f => !f.startsWith('.') && (f !== '__MACOSX'));
            // Final sanity: if we can detect extension files, proceed
            const hasExt = yield hasExtensionFiles(extractionPath);
            (0, log_1.log)('warn', 'Extraction completion monitoring timed out, but files exist - proceeding', {
                extractionPath,
            ...
            (0, log_1.log)('warn', 'Extraction completion monitoring timed out, and directory check failed', {
                extractionPath,
                maxWaitMs: effectiveMaxWaitMs,
                lastFileCount,
                hasFoundFiles,
 ...
 */
function validateExtractionCompleteness(extractionPath_1) {
    return __awaiter(this, arguments, void 0, function* (extractionPath, maxWaitMs = 10000) {
        const platformFactor = (process.platform === 'darwin') ? 1.5 : 1;
        const effectiveMaxWaitMs = Math.ceil(maxWaitMs * platformFactor);
        const startTime = Date.now();
        ...
        let checkInterval = initialCheckInterval;
        (0, log_1.log)('debug', 'Starting extraction completeness validation', {
            extractionPath,
            maxWaitMs: effectiveMaxWaitMs,
            platform: process.platform
        });
            ...
            try {
                ...
                if (presentKeyFiles.length > 0) {
                    ...
                    for (const file of presentKeyFiles) {
                        try {
                            yield fs.statAsync(path.join(extractionPath, file));
                        }
                        catch (accessErr) {
                            allAccessible = false;
                            break;
                    ...
                    if (allAccessible) {
                        (0, log_1.log)('debug', 'Key files are present and accessible, extraction is complete', {
                            extractionPath,
                            waitedMs: Date.now() - startTime,
                            platform: process.platform,
                ...
                if (hasJsFiles) {
                    ...
                    if (jsFile) {
                        try {
                            yield fs.statAsync(path.join(extractionPath, jsFile));
                            (0, log_1.log)('debug', 'JS files are present and accessible, extraction is complete', {
                                extractionPath,
                                waitedMs: Date.now() - startTime,
                                platform: process.platform,
                                sampleFile: jsFile
                    ...
                    try {
                        ...
                        if (entryStat.isDirectory()) {
                            ...
                            if (presentSubKeys.length > 0) {
                                ...
                                if (subAllAccessible) {
                                    (0, log_1.log)('debug', 'Key files found in subdirectory, extraction complete', {
                                        extractionPath,
                                        subdir: entry,
                                        keyFiles: presentSubKeys,
                                        waitedMs: Date.now() - startTime,
                            ...
                            if (subJs) {
                                try {
                                    yield fs.statAsync(path.join(entryPath, subJs));
                                    (0, log_1.log)('debug', 'JS file accessible in subdirectory, extraction complete', {
                                        extractionPath,
                                        subdir: entry,
                                        sampleFile: subJs,
                                        waitedMs: Date.now() - startTime,
                ...
                if (err.code === 'ENOENT') {
                    ...
                    (0, log_1.log)('debug', 'Error during completeness validation, continuing to wait', {
                        extractionPath,
                        error: err.message
                    });
                    yield new Promise(resolve => setTimeout(resolve, checkInterval));
        ...
        try {
            const hasExt = yield hasExtensionFiles(extractionPath);
            if (hasExt) {
                (0, log_1.log)('warn', 'Completeness validation timed out, but extension files detected - proceeding', {
                    extractionPath,
                    maxWaitMs: effectiveMaxWaitMs,
                    platform: process.platform,
                });
        ...
        (0, log_1.log)('warn', 'Extraction completeness validation timed out, proceeding anyway', {
            extractionPath,
            maxWaitMs: effectiveMaxWaitMs,
            platform: process.platform
        });
 ...
 * Enhanced with retry mechanisms for better reliability
 */
function validateExtensionFiles(extensionPath, info) {
    return __awaiter(this, void 0, void 0, function* () {
        (0, log_1.log)('debug', 'Starting comprehensive file validation', { extensionPath, extensionId: info.id });
        // Enhanced validation with retry mechanisms
        const validateDirectoryExists = () => retryValidationOperation(() => __awaiter(this, void 0, void 0, function* () {
            try {
                const stat = yield fs.statAsync(extensionPath);
                if (!stat.isDirectory()) {
                    throw new CustomErrors_1.DataInvalid(`Extension path is not a directory: ${extensionPath}`);
                }
                return stat;
            }
            catch (err) {
                throw new CustomErrors_1.DataInvalid(`Extension directory not found or not accessible: ${extensionPath} - ${err.message}`);
            }
        }), 8, // Increased retries for better reliability on macOS
        500, // Increased base delay for macOS file system
        5000, // 5s max delay
        `validate directory existence for ${extensionPath}`, (error) => {
            const errorMessage = error.message.toLowerCase();
            return (errorMessage.includes('enoent') || // File not found
                errorMessage.includes('ebusy') || // Resource busy
        ...
        const validateInfoJson = () => retryValidationOperation(() => __awaiter(this, void 0, void 0, function* () {
            const infoPath = path.join(extensionPath, 'info.json');
            try {
                yield fs.statAsync(infoPath);
                (0, log_1.log)('debug', 'info.json found and accessible', { infoPath });
                return infoPath;
            }
            catch (err) {
                throw new CustomErrors_1.DataInvalid(`info.json not found or not accessible: ${infoPath} - ${err.message}`);
            }
        }), 8, // Increased retries for better reliability on macOS
        500, // Increased base delay for macOS file system
        5000, // 5s max delay
        `validate info.json for ${extensionPath}`, (error) => {
            const errorMessage = error.message.toLowerCase();
            return (errorMessage.includes('enoent') || // File not found
                errorMessage.includes('ebusy') || // Resource busy
                errorMessage.includes('eperm') || // Permission denied
        ...
        const validateEntryScript = () => retryValidationOperation(() => __awaiter(this, void 0, void 0, function* () {
            const entryScript = yield findEntryScript(extensionPath);
            if (!entryScript) {
                throw new CustomErrors_1.DataInvalid(`No valid entry script found in extension: ${extensionPath}`);
            }
            (0, log_1.log)('debug', 'Entry script found and accessible', { entryScript, extensionPath });
            return entryScript;
        }), 8, // Increased retries for better reliability on macOS
        500, // Increased base delay for macOS file system
        5000, // 5s max delay
        `validate entry script for ${extensionPath}`, (error) => {
            const errorMessage = error.message.toLowerCase();
            return (errorMessage.includes('enoent') || // File not found
                errorMessage.includes('ebusy') || // Resource busy
                errorMessage.includes('eperm') || // Permission denied
        ...
        try {
            yield validateDirectoryExists();
            yield validateInfoJson();
            yield validateEntryScript();
        }
        catch (err) {
            (0, log_1.log)('error', 'File validation failed', {
                extensionPath,
                extensionId: info.id,
                error: err.message,
                code: err.code
        ...
        if (info.type === 'theme') {
            ...
            const validateThemeFiles = () => retryValidationOperation(() => __awaiter(this, void 0, void 0, function* () {
                let hasThemeFile = false;
                for (const themeFile of themeFiles) {
                    try {
                        yield fs.statAsync(path.join(extensionPath, themeFile));
                        hasThemeFile = true;
                        break;
                    }
                    catch (err) {
                ...
                if (!hasThemeFile) {
                    (0, log_1.log)('warn', 'Theme extension missing expected theme files', { extensionPath, expectedFiles: themeFiles });
            ...
            yield validateThemeFiles();
        }
        (0, log_1.log)('debug', 'Comprehensive file validation completed successfully', { extensionPath, extensionId: info.id });
    });
}
function removeOldVersion(api, info) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (previousVersions.length > 0) {
            (0, log_1.log)('info', '🗑️ removing previous versions of the extension', {
                previousVersions,
                newPath: info.path,
        ...
        for (const key of previousVersions) {
            api.store.dispatch((0, actions_1.removeExtension)(key));
            // Actually delete the physical files from disk
            const extensionPath = installed[key].path;
            if (extensionPath) {
                try {
                    if (yield fs.statAsync(extensionPath).then(() => true, () => false)) {
                        (0, log_1.log)('info', 'deleting extension files', { path: extensionPath });
                        yield fsExtra.remove(extensionPath);
                        (0, log_1.log)('info', 'extension files deleted successfully', { path: extensionPath });
                    }
                }
                catch (err) {
                    (0, log_1.log)('warn', 'failed to delete extension files', { path: extensionPath, error: err.message });
...
function validateTheme(extPath) {
    return Promise.resolve(fs.readdirAsync(extPath))
        .then(fileNames => (0, promise_helpers_1.promiseFilter)(fileNames, (fileName) => fs.statAsync(path.join(extPath, fileName))
        .then(stats => stats.isDirectory())))
        .then(dirNames => {
        if (dirNames.length === 0) {
            return Promise.reject(new CustomErrors_1.DataInvalid('Expected a subdirectory containing the stylesheets'));
        }
        // Convert Bluebird.map to standard Promise.all with map
        const validationPromises = dirNames.map(dirName => retryValidationOperation(() => Promise.resolve(fs.readdirAsync(path.join(extPath, dirName)).then(files => {
            if (!files.includes('variables.scss')
                && !files.includes('style.scss')
                && !files.includes('fonts.scss')) {
                return Promise.reject(new CustomErrors_1.DataInvalid('Theme not found'));
            }
            else {
                return Promise.resolve();
            }
...
function validateTranslation(extPath) {
    return Promise.resolve(fs.readdirAsync(extPath))
        .then(fileNames => (0, promise_helpers_1.promiseFilter)(fileNames, (fileName) => Promise.resolve(isLocaleCode(fileName))))
        .then(filteredFileNames => (0, promise_helpers_1.promiseFilter)(filteredFileNames, (fileName) => fs.statAsync(path.join(extPath, fileName))
        .then(stats => stats.isDirectory())))
        .then(dirNames => {
        if (dirNames.length !== 1) {
            return Promise.reject(new CustomErrors_1.DataInvalid('Expected exactly one language subdirectory'));
        }
        ...
        if (!(0, languagemap_1.languageExists)(language)
            ...
            return Promise.reject(new CustomErrors_1.DataInvalid('Directory isn\'t a language code'));
        }
        return retryValidationOperation(() => Promise.resolve(fs.readdirAsync(path.join(extPath, dirNames[0])).then(files => {
            if (files.find(fileName => path.extname(fileName) === '.json') === undefined) {
                return Promise.reject(new CustomErrors_1.DataInvalid('No translation files'));
            }
            return Promise.resolve();
        })), 5, // retries
        300, // base delay
        3000, // max delay
        `validate translation directory ${dirNames[0]} for ${extPath}`).then(() => undefined); // Convert to standard Promise
    })
        .then(() => undefined);
}
/**
...
function validateExtension(extPath) {
    return retryValidationOperation(() => Promise.resolve(fs.statAsync(path.join(extPath, 'info.json')))
        .then(() => findEntryScript(extPath))
        .then(entry => {
        if (entry) {
            return Promise.resolve();
        }
        return Promise.reject(new CustomErrors_1.DataInvalid('Extension needs to include an entry script (index.js or package.json main) and info.json on top-level'));
    }), 8, // retries
    500, // base delay
    5000, // max delay
    `validateExtension for ${extPath}`).catch(err => {
        if (err.code === 'ENOENT') {
            return Promise.reject(new CustomErrors_1.DataInvalid('Extension needs to include an entry script (index.js or package.json main) and info.json on top-level'));
        }
        return Promise.reject(err);
...
function validateInstall(api, extPath, info) {
    if (info === undefined) {
        ...
        return retryValidationOperation(() => validateTheme(extPath), 5, // retries
        500, // base delay
        3000, // max delay
        `validateTheme for ${extPath}`)
            .catch(err => {
            if (err instanceof CustomErrors_1.DataInvalid) {
                validAsTheme = false;
            }
            else {
                return Promise.reject(err);
            ...
            .then(() => retryValidationOperation(() => validateTranslation(extPath), 5, // retries
        500, // base delay
        3000, // max delay
        `validateTranslation for ${extPath}`))
            ...
            .then(() => retryValidationOperation(() => validateExtension(extPath), 5, // retries
        500, // base delay
        3000, // max delay
        `validateExtension for ${extPath}`))
            ...
            if (!validAsExtension && !validAsTheme && !validAsTranslation) {
                return Promise.reject(new CustomErrors_1.DataInvalid('Doesn\'t seem to contain a correctly packaged extension, '
            ...
            if (validAsExtension) {
                try {
                    ...
                    if (fsNode.existsSync(infoPath)) {
                        ...
                        (0, log_1.log)('debug', 'Checking if extension is a game extension', {
                        ...
                        if (extensionInfo.name &&
                            ...
                            (0, log_1.log)('info', 'Identified potential game extension during installation', {
                                extensionName: extensionInfo.name,
                                path: extPath,
                            ...
                            (0, log_1.log)('debug', 'Setting extension type to game and emitting refresh event', {
                                extensionName: extensionInfo.name,
                                path: extPath
                            });
                            ...
                            (0, log_1.log)('debug', 'Extension does not match game extension patterns', {
                                extensionName: extensionInfo.name,
                        ...
                        (0, log_1.log)('warn', 'info.json not found for extension', { path: infoPath });
                    }
                }
                catch (parseErr) {
                    (0, log_1.log)('warn', 'Failed to parse info.json for game extension detection', {
                        path: extPath,
        ...
        return retryValidationOperation(() => validateTheme(extPath).then(() => Promise.resolve('theme')), 5, // retries
        500, // base delay
        3000, // max delay
        `validateTheme with type for ${extPath}`);
    }
    else if (info.type === 'translation') {
        return retryValidationOperation(() => validateTranslation(extPath).then(() => Promise.resolve('translation')), 5, // retries
        500, // base delay
        3000, // max delay
        `validateTranslation with type for ${extPath}`);
    }
    else {
        return retryValidationOperation(() => validateExtension(extPath).then(() => Promise.resolve(undefined)), 5, // retries
        500, // base delay
        3000, // max delay
...
function installExtension(api, archivePath, info) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        (0, log_1.log)('info', 'Starting extension installation process', {
            archivePath,
            extensionsPath,
            tempPath,
            platform: process.platform,
        ...
        return Promise.resolve(fs.ensureDirAsync(extensionsPath))
            .then(() => {
            (0, log_1.log)('debug', 'Ensured extensions directory exists', { extensionsPath });
            return fs.ensureDirAsync(tempPath);
        })
            .then(() => __awaiter(this, void 0, void 0, function* () {
            var _a;
            (0, log_1.log)('debug', 'Ensured temporary directory exists', { tempPath });
            ...
            const predicted = (process.platform === 'darwin' && (lower.endsWith('.zip'))) ? 'macOS-zip-ditto'
                : ((process.platform === 'darwin') && (lower.endsWith('.7z') || lower.endsWith('.rar'))) ? 'macOS-7z/rar-native'
                    : ((lower.endsWith('.tar') || lower.endsWith('.tar.gz') || lower.endsWith('.tgz'))) ? 'tar'
                        : 'node-7z';
            (0, log_1.log)('info', 'Predicted extraction method', {
                archivePath,
                tempPath,
                platform: process.platform,
                predicted,
            ...
            if (process.platform === 'darwin') {
                ...
                const modsSettings = ((_a = state === null || state === void 0 ? void 0 : state.settings) === null || _a === void 0 ? void 0 : _a.mods) || {};
                const removeQuarantine = (modsSettings.macOSRemoveQuarantine !== false);
                yield retryValidationOperation(() => extractArchive(archivePath, tempPath, { ssc: false, removeQuarantine }), 3, // a few retries around the core extractor
                400, // base delay
                3000, // max delay
                `macOS extractArchive for ${archivePath}`, (error) => {
                ...
                yield extractArchive(archivePath, tempPath, { ssc: false });
            }
            const extractionDuration = Date.now() - extractionStartTime;
            (0, log_1.log)('info', 'Archive extraction completed', {
                tempPath,
                archivePath,
                platform: process.platform,
            ...
            (0, log_1.log)('debug', 'Monitoring extraction completion', { tempPath, archivePath });
            yield waitForExtractionCompletion(tempPath, 5000);
            (0, log_1.log)('debug', 'Validating extraction completeness', { tempPath, archivePath });
            yield validateExtractionCompleteness(tempPath, 20000);
            (0, log_1.log)('debug', 'Adding additional delay for file system settlement', { delay: '500ms' });
            yield new Promise(resolve => setTimeout(resolve, 500));
        }))
            .then(() => {
            (0, log_1.log)('debug', 'Flattening nested root directory if needed', { tempPath });
            return flattenNestedRoot(tempPath);
        })
            .then(() => {
            (0, log_1.log)('debug', 'Reading extension info from extracted files', { tempPath });
            return (0, util_2.readExtensionInfo)(tempPath, false, info);
        })
            ...
            .then(manifestInfo => {
            (0, log_1.log)('debug', 'Merging extension info', {
                providedInfo: !!info,
            ...
            .catch(err => {
            (0, log_1.log)('error', 'Failed to read or merge extension info', {
                error: err.message,
                code: err.code,
                archivePath,
                tempPath
            ...
            if (err && err.code === 'ENOENT') {
                if (info !== undefined) {
                    (0, log_1.log)('debug', 'Using provided info as fallback', { archivePath });
                    return {
        ...
        fs.writeFileAsync(path.join(tempPath, 'info.json'), JSON.stringify(manifestInfo.info, undefined, 2))
            .then(() => {
            (0, log_1.log)('debug', 'Updated info.json with merged information', {
                tempPath,
                extensionId: manifestInfo.id
            });
            return manifestInfo;
            ...
            (0, log_1.log)('info', 'Processing extension installation', {
                extensionId: manifestInfo.id,
            ...
            if (manifestInfo.info.type !== undefined) {
                ...
                (0, log_1.log)('debug', 'Extension type from manifest info', {
                    extensionId: extName,
                    type: type
                });
            }
            // Determine whether this is a manifest-only update (no acceptable entry script in archive)
            return retryValidationOperation(() => findEntryScript(tempPath), 8, // Increased retries for better reliability on macOS
            500, // Increased base delay for macOS file system
            5000, // 5s max delay
            `findEntryScript for temp path ${tempPath}`, (error) => {
                const errorMessage = error.message.toLowerCase();
                return (errorMessage.includes('enoent') || // File not found
                    errorMessage.includes('ebusy') || // Resource busy
                    errorMessage.includes('eperm') || // Permission denied
                ...
                if (!tempEntry) {
                    (0, log_1.log)('debug', 'No entry script found in archive, checking for existing installation', {
                        tempPath,
                        destPath
                    });
                    // Enhanced retry mechanism for finding entry script in existing installation
                    return retryValidationOperation(() => findEntryScript(destPath), 8, // Increased retries for better reliability on macOS
                    500, // Increased base delay for macOS file system
                    5000, // 5s max delay
                    `findEntryScript for dest path ${destPath}`, (error) => {
                        const errorMessage = error.message.toLowerCase();
                        return (errorMessage.includes('enoent') || // File not found
                            errorMessage.includes('ebusy') || // Resource busy
                            errorMessage.includes('eperm') || // Permission denied
                        ...
                        if (destEntry) {
                            (0, log_1.log)('info', 'Manifest-only update detected', {
                                extensionId: extName,
                                destPath
                            });
                            manifestOnly = true;
                            ...
                            if (process.platform === 'darwin') {
                                (0, log_1.log)('debug', 'Adding additional delay for manifest-only update on macOS', {
                                    delay: '1000ms',
                                    platform: process.platform
                                });
                                additionalDelayPromise = new Promise(resolve => setTimeout(resolve, 1000));
                            }
                            ...
                            return additionalDelayPromise
                                ...
                                .then(() => {
                                (0, log_1.log)('debug', 'Copied updated info.json to existing installation', { destPath });
                                return fs.removeAsync(tempPath);
                            })
                                .then(() => {
                                (0, log_1.log)('debug', 'Removed temporary directory', { tempPath });
                                return undefined;
                            });
                        }
                        else {
                            (0, log_1.log)('error', 'No entry script found in archive or existing installation after all retries', {
                                tempPath,
                                destPath
                            });
                            // No entry in archive and no existing installed extension to update
                            throw new CustomErrors_1.DataInvalid('Extension package missing entry script (expected index.js or package.json main)');
                        }
                    });
                }
                else {
                    (0, log_1.log)('debug', 'Entry script found in archive', {
                        tempPath,
                ...
                if (!manifestOnly) {
                    (0, log_1.log)('debug', 'Validating full extension installation', { tempPath });
                    return retryValidationOperation(() => validateInstall(api, tempPath, info), 5, // 5 retries for better reliability
                    300, // 300ms base delay
                    3000, // 3s max delay
                    `validateInstall for ${tempPath}`).then(guessedType => {
                        if (type === undefined) {
                            type = guessedType;
                            (0, log_1.log)('debug', 'Determined extension type', {
                                extensionId: extName,
                                type: guessedType
                            });
                        }
                        else {
                            (0, log_1.log)('debug', 'Extension type already set, not overriding', {
                                extensionId: extName,
                                existingType: type,
                        ...
                        if (type !== undefined && manifestInfo.info.type !== type) {
                            ...
                            (0, log_1.log)('debug', 'Updating manifest info with determined type', {
                                extensionId: extName,
                                oldType: manifestInfo.info.type,
                                newType: type
                            });
                            // Update the info.json file with the correct type
                            return fs.writeFileAsync(path.join(tempPath, 'info.json'), JSON.stringify(manifestInfo.info, undefined, 2))
                                .then(() => {
                                (0, log_1.log)('debug', 'Updated info.json with determined extension type', {
                                    tempPath,
                                    extensionId: extName,
                                    type: type
                                });
            ...
            .then((infoForRemoval) => {
            if (manifestOnly) {
                (0, log_1.log)('debug', 'Skipping old version removal for manifest-only update', { extName });
                return Promise.resolve();
            }
            (0, log_1.log)('debug', 'Removing old version of extension if it exists', { extName });
            return removeOldVersion(api, infoForRemoval);
        })
            .then(() => {
            if (manifestOnly) {
                (0, log_1.log)('debug', 'Skipping destination directory removal for manifest-only update', { destPath });
                return Promise.resolve();
            }
            (0, log_1.log)('debug', 'Removing destination directory if it exists', { destPath });
            return fs.removeAsync(destPath);
        })
            .then(() => {
            if (manifestOnly) {
                (0, log_1.log)('debug', 'Skipping directory rename for manifest-only update', { tempPath, destPath });
                return Promise.resolve();
            }
            (0, log_1.log)('debug', 'Renaming temporary directory to destination', { tempPath, destPath });
            return fs.renameAsync(tempPath, destPath);
        })
            .then(() => {
            if (manifestOnly) {
                (0, log_1.log)('info', 'Manifest-only update completed successfully', { extName });
                return Promise.resolve();
            }
            (0, log_1.log)('debug', 'Performing comprehensive file validation after extraction and file operations', { destPath });
            ...
            return fs.readFileAsync(path.join(destPath, 'info.json'), { encoding: 'utf8' })
                ...
                .then((extensionInfo) => {
                (0, log_1.log)('debug', 'Validating extension files', {
                    destPath,
                    extensionId: extensionInfo.id,
                    extensionName: extensionInfo.name,
                ...
                if (type !== undefined && extensionInfo.type !== type) {
                    ...
                    (0, log_1.log)('debug', 'Updating final info.json with correct extension type', {
                        destPath,
                        extensionId: extensionInfo.id,
                    ...
                    // Update the info.json file with the correct type
                    return fs.writeFileAsync(path.join(destPath, 'info.json'), JSON.stringify(extensionInfo, undefined, 2))
                        .then(() => {
                        (0, log_1.log)('debug', 'Updated final info.json with extension type', {
                            destPath,
                            extensionId: extensionInfo.id,
                            type: type
                        });
                ...
                (0, log_1.log)('error', 'Extension file validation failed after installation', {
                    destPath,
                    type,
            ...
            if (type === 'translation') {
                (0, log_1.log)('debug', 'Processing translation extension', { destPath });
                return fs.readdirAsync(destPath)
                    .then(entries => (0, promise_helpers_1.promiseMap)(entries, (entry) => fs.statAsync(path.join(destPath, entry))
                    .then(stat => ({ name: entry, stat })))).then(() => {
                    (0, log_1.log)('debug', 'Translation extension processing completed', { destPath });
                    return undefined;
                });
            }
            else if (type === 'theme') {
                (0, log_1.log)('debug', 'Processing theme extension', { destPath });
                return Promise.resolve();
            }
            else if (type === 'game') {
                (0, log_1.log)('debug', 'Processing game extension', {
                    destPath,
                ...
                if (api.getLoadedExtensions().find(ext => ext.name === extName) === undefined) {
                    (0, log_1.log)('debug', 'Installing extension dependencies', { destPath, extName });
                    return installExtensionDependencies(api, destPath);
                }
                else {
                    (0, log_1.log)('debug', 'Skipping dependency installation - extension already loaded', { extName });
                    return Promise.resolve();
                }
            }
        })
            ...
            (0, log_1.log)('debug', 'updating extension list synchronously after installation', {
                extName,
                platform: process.platform,
                destPath
            });
            const updateExtensionList = () => __awaiter(this, void 0, void 0, function* () {
                try {
                    // Verify the extension directory exists and is accessible
                    yield fsExtra.access(destPath);
                    // Additional delay on macOS to ensure file system operations are fully settled
                    if (process.platform === 'darwin') {
                        (0, log_1.log)('debug', 'Adding additional delay for macOS file system settlement', {
                            delay: '1000ms',
                            platform: process.platform
                        });
                    ...
                    (0, log_1.log)('debug', 'Reading extensions synchronously', { force: true });
                    const extensions = (0, util_2.readExtensionsSync)(true);
                    // Verify our extension is in the list
                    if (!extensions[extName]) {
                        (0, log_1.log)('error', 'Extension not found in extension list after installation', {
                            extName,
                    ...
                    api.store.dispatch((0, actions_2.setInstalledExtensions)(extensions));
                    (0, log_1.log)('info', 'Extension list updated synchronously after installation', {
                        extensionCount: Object.keys(extensions).length,
                    ...
                    (0, log_1.log)('warn', 'Failed to update extension list synchronously', {
                        error: err.message,
                        extName,
                        platform: process.platform,
                        destPath
                    ...
                    for (let retry = 0; retry < maxRetries; retry++) {
                        ...
                        (0, log_1.log)('debug', `Retrying extension list update in ${delay}ms`, {
                        ...
                        try {
                            ...
                            if (extensions[extName]) {
                                api.store.dispatch((0, actions_2.setInstalledExtensions)(extensions));
                                (0, log_1.log)('info', 'Extension list updated on retry', {
                                    extensionCount: Object.keys(extensions).length,
                                    installedExtension: extName,
                                    retryAttempt: retry + 1
                                });
                            ...
                            if (retry === maxRetries - 1) {
                                (0, log_1.log)('error', 'Failed to update extension list even after all retries', {
                                ...
                                (0, log_1.log)('debug', 'Extension list update retry failed, trying again', {
                                    error: retryErr.message,
                                    retryAttempt: retry + 1,
            ...
            .catch(err => {
            (0, log_1.log)('error', 'Extension installation failed', {
                error: err.message,
                stack: err.stack,
                code: err.code,
                archivePath,
            ...
            try {
                ...
                (0, log_1.log)('debug', 'Cleaned up temporary directory after failure', { tempPath });
            }
            catch (removeErr) {
 ...
 */
function findEntryScript(extPath) {
    // Enhanced tryExists with retry mechanism for macOS timing issues
    const tryExists = (p) => retryValidationOperation(() => Promise.resolve(fs.statAsync(p)).then(() => true).catch(() => false), 5, // retries
    300, // base delay
    3000, // max delay
    `check file existence for ${p}`);
    // Enhanced tryReadPkgMain with retry mechanism
    const tryReadPkgMain = () => retryValidationOperation(() => Promise.resolve(fs.readFileAsync(path.join(extPath, 'package.json'), { encoding: 'utf8' }))
        .then(raw => {
        try {
        ...
        .catch(() => undefined), 5, // retries
    300, // base delay
    3000, // max delay
    `read package.json for ${extPath}`);
    const attemptFind = () => __awaiter(this, void 0, void 0, function* () {
        ...
        for (const pattern of searchPatterns) {
            ...
            const exists = yield tryExists(filePath);
            if (exists) {
                return filePath;
            }
        }
        // 2. Try package.json main field in root
        const rootMain = yield tryReadPkgMain();
        if (rootMain) {
            return rootMain;
        }
        ...
        for (const subDir of subDirs) {
            ...
            try {
                const exists = yield tryExists(subDirPath);
                if (exists) {
                    const subMain = yield tryReadPkgMainInDir(subDirPath);
                    if (subMain) {
                        return subMain;
                    }
                }
        ...
        try {
            const jsFiles = yield findJsFilesRecursive(extPath, 0, 3);
    ...
    const tryReadPkgMainInDir = (dirPath) => retryValidationOperation(() => Promise.resolve(fs.readFileAsync(path.join(dirPath, 'package.json'), { encoding: 'utf8' }))
        .then(raw => {
        try {
            const pkg = JSON.parse(raw);
            if (pkg && typeof pkg.main === 'string' && pkg.main.length > 0) {
    ...
    const findJsFilesRecursive = (dir, depth, maxDepth) => __awaiter(this, void 0, void 0, function* () {
        if (depth > maxDepth)
            return [];
        const files = [];
        try {
            const entries = yield fs.readdirAsync(dir);
            for (const entry of entries) {
                const fullPath = path.join(dir, entry);
                try {
                    const stat = yield fs.statAsync(fullPath);
                    if (stat.isFile() && entry.endsWith('.js')) {
                        files.push(fullPath);
                    }
                    else if (stat.isDirectory() && !entry.startsWith('.') && entry !== 'node_modules') {
                        const subFiles = yield findJsFilesRecursive(fullPath, depth + 1, maxDepth);
                        files.push(...subFiles);
                    }
                }
                catch (_a) {
    ...
    return retryValidationOperation(() => Promise.resolve(attemptFind()), 8, // Increased retries for better reliability on macOS
    500, // Increased base delay for macOS file system
    5000, // 5s max delay
    `findEntryScript for ${extPath}`, (error) => {
        const errorMessage = error.message.toLowerCase();
        return (
        // Common file system timing issues on macOS
        errorMessage.includes('enoent') || // File not found
...
function tryCommandsSync(cmds, cwd) {
    const attemptedCommands = [];
    const startTime = Date.now();
    (0, log_1.log)('debug', 'Starting extraction command attempts', {
    ...
    for (let idx = 0; idx < cmds.length; idx++) {
        ...
        (0, log_1.log)('debug', `Attempting extraction command ${idx + 1}/${cmds.length}`, {
        ...
        if (!isCommandAvailable) {
            ...
            (0, log_1.log)('debug', 'Skipping unavailable command', {
                command: commandString,
                error: notFoundError,
        ...
        try {
            ...
            if (!hasArchivePath || !hasDestPath) {
                (0, log_1.log)('warn', 'Command arguments may be incomplete', {
                    command: commandString,
                    hasArchivePath,
                    hasDestPath,
                    args
            ...
            (0, log_1.log)('debug', 'Executing extraction command', {
                command: commandString,
                attempt: idx + 1,
                totalAttempts: cmds.length
            });
            // Use retry logic for the extraction command
            withRetry(() => runCommandSync(cmd, args, cwd), DEFAULT_RETRY_CONFIG, `7z extraction: ${commandString}`);
            const duration = Date.now() - attemptStartTime;
            const totalDuration = Date.now() - startTime;
            (0, log_1.log)('info', 'Successfully executed extraction command', {
                command: commandString,
                attempt: idx + 1,
            ...
            if (cwd && fsNode.existsSync(cwd)) {
                try {
                    ...
                    (0, log_1.log)('debug', 'Post-extraction directory contents', {
                        extractionPath: cwd,
                    ...
                    (0, log_1.log)('warn', 'Could not verify extraction results', {
                        extractionPath: cwd,
            ...
            const errorDetails = {
                ...
                exitCode: err === null || err === void 0 ? void 0 : err.exitCode,
                ...
                signal: err === null || err === void 0 ? void 0 : err.signal
            };
            (0, log_1.log)('warn', 'Archive extraction attempt failed', errorDetails);
            attemptInfo.error = (err === null || err === void 0 ? void 0 : err.message) || 'Unknown error';
            attemptInfo.exitCode = err === null || err === void 0 ? void 0 : err.exitCode;
            attemptedCommands.push(attemptInfo);
            // Analyze error for common issues
            const errorMessage = ((err === null || err === void 0 ? void 0 : err.message) || '').toLowerCase();
            const stderr = ((err === null || err === void 0 ? void 0 : err.stderr) || '').toLowerCase();
            const combinedError = `${errorMessage} ${stderr}`;
            if (combinedError.includes('permission denied') || combinedError.includes('eperm')) {
                (0, log_1.log)('warn', 'Permission issue detected', {
                    command: commandString,
                ...
                (0, log_1.log)('warn', 'File not found issue detected', {
                    command: commandString,
                ...
                (0, log_1.log)('warn', 'Archive corruption suspected', {
                    command: commandString,
                ...
                (0, log_1.log)('warn', 'Disk space issue suspected', {
                    command: commandString,
            ...
            if (idx === cmds.length - 1) {
                ...
                const detailedError = new Error([
                    ...
                    '',
                    `🔧 Attempted Commands:`,
                    ...attemptedCommands.map((cmd, i) => `   ${i + 1}. ${cmd.available ? '✅' : '❌'} ${cmd.command}${cmd.error ? ` → ${cmd.error}` : ''}`),
                    '',
                ...
                (0, log_1.log)('error', 'All archive extraction attempts failed - comprehensive diagnostic', diagnosticInfo);
                throw detailedError;
            }
...
function getPackaged7zPath() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            // Base node_modules path (handles dev and production/asar-unpacked)
            const modulesBase = (0, getVortexPath_1.default)('modules_unpacked');
            const candidates = [];
            ...
            for (const p of candidates) {
                try {
                    const st = yield fs.statAsync(p);
                    if (st && st.isFile()) {
                        // Ensure executable permissions on Unix
                        if (process.platform !== 'win32') {
                            try {
                                yield fs.chmodAsync(p, 0o755);
                            }
                            catch (_) { /* ignore */ }
                        }
                        return p;
            ...
            if (process.platform === 'darwin') {
                ...
                try {
                    ...
                    if (sevenZipBin) {
                        ...
                        if (sevenZipBinPath) {
                            try {
                                const st = yield fs.statAsync(sevenZipBinPath);
                                if (st && st.isFile()) {
                                    try {
                                        yield fs.chmodAsync(sevenZipBinPath, 0o755);
                                    }
                                    catch (_) { /* ignore */ }
                ...
                try {
                    ...
                    if (sevenBinPath) {
                        try {
                            const st = yield fs.statAsync(sevenBinPath);
                            if (st && st.isFile()) {
                                try {
                                    yield fs.chmodAsync(sevenBinPath, 0o755);
                                }
                                catch (_) { /* ignore */ }
                                return sevenBinPath;
                            }
                            ...
                            if (sevenBinPath.includes('/darwin/')) {
                                ...
                                try {
                                    const st = yield fs.statAsync(correctedPath);
                                    if (st && st.isFile()) {
                                        try {
                                            yield fs.chmodAsync(correctedPath, 0o755);
                                        }
                                        catch (_) { /* ignore */ }
 ...
 */
function withRetry(operation, config = DEFAULT_RETRY_CONFIG, operationName = 'operation') {
    let lastError;
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
        try {
            (0, log_1.log)('debug', `Attempting ${operationName}`, {
                attempt,
            ...
            if (attempt > 1) {
                (0, log_1.log)('info', `${operationName} succeeded after retry`, {
                    attempt,
                    totalAttempts: attempt,
            ...
            lastError = error;
            (0, log_1.log)('warn', `${operationName} failed on attempt ${attempt}`, {
                attempt,
                maxAttempts: config.maxAttempts,
                error: error.message,
            ...
            if (attempt < config.maxAttempts) {
                ...
                (0, log_1.log)('debug', `Waiting before retry`, {
                    delayMs: delay,
    ...
    (0, log_1.log)('error', `${operationName} failed after all retry attempts`, {
...
function validate7zBinarySync(binaryPath) {
    const startTime = Date.now();
    (0, log_1.log)('debug', 'Validating 7z binary functionality', {
        binaryPath,
        exists: fsNode.existsSync(binaryPath)
    });
    if (!fsNode.existsSync(binaryPath)) {
        (0, log_1.log)('debug', '7z binary validation failed: file does not exist', { binaryPath });
        return false;
    }
    try {
        ...
        (0, log_1.log)('debug', '7z binary validation successful', {
            binaryPath,
            duration: `${duration}ms`,
        ...
        if (err.exitCode !== undefined) {
            ...
            if (err.stdout || err.stderr) {
                (0, log_1.log)('debug', '7z binary validation successful (non-zero exit but has output)', {
                    binaryPath,
                ...
                (0, log_1.log)('warn', '7z binary validation failed: no output produced', {
                    binaryPath,
                    exitCode: err.exitCode,
                    duration: `${duration}ms`,
            ...
            (0, log_1.log)('warn', '7z binary validation failed: execution error', {
                binaryPath,
                error: err.message,
                duration: `${duration}ms`,
...
function getPackaged7zPathSync() {
    const startTime = Date.now();
    (0, log_1.log)('debug', 'Starting bundled 7z tool resolution', {
        platform: process.platform,
        arch: process.arch,
        nodeVersion: process.version
    });
    try {
        // Base node_modules path (handles dev and production/asar-unpacked)
        const modulesBase = (0, getVortexPath_1.default)('modules_unpacked');
        (0, log_1.log)('debug', 'Resolved modules base path', {
            modulesBase,
            exists: fsNode.existsSync(modulesBase)
        });
        if (!fsNode.existsSync(modulesBase)) {
            (0, log_1.log)('warn', 'Modules base directory does not exist', {
                modulesBase,
        ...
        if (process.platform === 'win32') {
            ...
            const systemArch = (0, macOSGameCompatibility_1.getMacOSArchitecture)();
        ...
        (0, log_1.log)('debug', 'Generated candidate paths for bundled 7z tools', {
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            (0, log_1.log)('debug', `Checking candidate ${i + 1}/${candidates.length}`, {
                path: p,
                package: p.includes('7zip-bin') ? '7zip-bin' : '7z-bin'
            });
            try {
                if (!fsNode.existsSync(p)) {
                    (0, log_1.log)('debug', 'Candidate file does not exist', {
                        path: p,
                ...
                if (st && st.isFile()) {
                    ...
                    if (st.size === 0) {
                        (0, log_1.log)('warn', 'Found zero-byte 7z binary', {
                            path: p,
                            size: st.size,
                    ...
                    (0, log_1.log)('debug', 'Found valid 7z binary candidate', {
                        path: p,
                        size: st.size,
                    ...
                    if (process.platform !== 'win32') {
                        try {
                            fsNode.chmodSync(p, 0o755);
                            (0, log_1.log)('debug', 'Set executable permissions on 7z binary', { path: p });
                        }
                        catch (chmodErr) {
                            (0, log_1.log)('warn', 'Failed to set executable permissions', {
                                path: p,
                    ...
                    if (validate7zBinarySync(p)) {
                        const duration = Date.now() - startTime;
                        (0, log_1.log)('info', 'Successfully resolved and validated bundled 7z tool via file system', {
                            path: p,
                            package: p.includes('7zip-bin') ? '7zip-bin' : '7z-bin',
                            method: 'filesystem',
                            duration: `${duration}ms`,
                        ...
                        (0, log_1.log)('warn', 'Found 7z binary but validation failed', {
                            path: p,
                            package: p.includes('7zip-bin') ? '7zip-bin' : '7z-bin',
                            size: st.size,
                    ...
                    (0, log_1.log)('debug', 'Candidate exists but is not a file', {
                        path: p,
                ...
                (0, log_1.log)('debug', 'Failed to stat candidate file', {
                    path: p,
        ...
        (0, log_1.log)('debug', 'No file system candidates found, trying package resolution');
        // As a last resort, try resolving via package exports (may point into asar-unpacked)
        // On macOS, prioritize 7zip-bin over 7z-bin since 7z-bin is broken
        if (process.platform === 'darwin') {
            // Try 7zip-bin package first on macOS
            try {
                (0, log_1.log)('debug', 'Attempting to resolve 7zip-bin package on macOS');
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const sevenZipBin = require('7zip-bin');
                if (sevenZipBin) {
                    // 7zip-bin exports an object with path7za property, not a string
                    const sevenZipBinPath = sevenZipBin.path7za || sevenZipBin;
                    if (sevenZipBinPath) {
                        (0, log_1.log)('debug', 'Found 7zip-bin package path', {
                        ...
                        try {
                            ...
                            if (st && st.isFile()) {
                                if (st.size === 0) {
                                    (0, log_1.log)('warn', 'Found zero-byte 7zip-bin binary', {
                                        path: sevenZipBinPath,
                                    ...
                                    try {
                                        fsNode.chmodSync(sevenZipBinPath, 0o755);
                                        (0, log_1.log)('debug', 'Set executable permissions on 7zip-bin binary', { path: sevenZipBinPath });
                                    }
                                    catch (chmodErr) {
                                        (0, log_1.log)('warn', 'Failed to set executable permissions on 7zip-bin', {
                                            path: sevenZipBinPath,
                                            error: chmodErr.message
                                        });
                                    }
                                    ...
                                    if (validate7zBinarySync(sevenZipBinPath)) {
                                        const duration = Date.now() - startTime;
                                        (0, log_1.log)('info', 'Successfully resolved and validated bundled 7z tool via 7zip-bin package', {
                                            path: sevenZipBinPath,
                                            package: '7zip-bin',
                                        ...
                                        (0, log_1.log)('warn', 'Found 7zip-bin package binary but validation failed', {
                                            path: sevenZipBinPath,
                                            package: '7zip-bin',
                                            size: st.size,
                            ...
                            (0, log_1.log)('debug', 'Failed to stat 7zip-bin package path', {
                                path: sevenZipBinPath,
                                error: statErr.message
                            });
                        }
                        ...
                        (0, log_1.log)('warn', '7zip-bin package found but no valid path property', {
                ...
                (0, log_1.log)('debug', 'Failed to require 7zip-bin package', {
            ...
            // Then try 7z-bin package as last resort on macOS
            try {
                (0, log_1.log)('debug', 'Attempting to resolve 7z-bin package on macOS (last resort)');
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const sevenBinPath = require('7z-bin');
                if (sevenBinPath) {
                    (0, log_1.log)('debug', 'Found 7z-bin package path', { path: sevenBinPath });
                    try {
                        const st = fs.statSync(sevenBinPath);
                        if (st && st.isFile()) {
                            try {
                                fsNode.chmodSync(sevenBinPath, 0o755);
                                (0, log_1.log)('debug', 'Set executable permissions on 7z-bin binary', { path: sevenBinPath });
                            }
                            catch (chmodErr) {
                                (0, log_1.log)('warn', 'Failed to set executable permissions on 7z-bin', {
                                    path: sevenBinPath,
                                    error: chmodErr.message
                                });
                            }
                            ...
                            if (validate7zBinarySync(sevenBinPath)) {
                                const duration = Date.now() - startTime;
                                (0, log_1.log)('info', 'Successfully resolved and validated bundled 7z tool via 7z-bin package', {
                                    path: sevenBinPath,
                                    package: '7z-bin',
                                    method: 'package-resolution',
                                    duration: `${duration}ms`,
                                ...
                                (0, log_1.log)('warn', 'Found 7z-bin package binary but validation failed', {
                                    path: sevenBinPath,
                                    package: '7z-bin',
                                    size: st.size,
                                    suggestion: 'Package binary may be corrupted'
                        ...
                        (0, log_1.log)('debug', 'Failed to stat 7z-bin package path, trying path correction', {
                            path: sevenBinPath,
                            error: statErr.message
                        });
                        // On macOS, the 7z-bin package may return a path like .../darwin/7z that doesn't exist
                        ...
                        if (sevenBinPath.includes('/darwin/')) {
                            const correctedPath = sevenBinPath.replace('/darwin/', '/bin/');
                            (0, log_1.log)('debug', 'Trying corrected path for 7z-bin on macOS', {
                            ...
                            try {
                                ...
                                if (st && st.isFile()) {
                                    try {
                                        fsNode.chmodSync(correctedPath, 0o755);
                                        (0, log_1.log)('debug', 'Set executable permissions on corrected 7z-bin path', { path: correctedPath });
                                    }
                                    catch (chmodErr) {
                                        (0, log_1.log)('warn', 'Failed to set executable permissions on corrected 7z-bin path', {
                                            path: correctedPath,
                                            error: chmodErr.message
                                        });
                                    }
                                    const duration = Date.now() - startTime;
                                    (0, log_1.log)('info', 'Successfully resolved bundled 7z tool via corrected 7z-bin path', {
                                        path: correctedPath,
                                        package: '7z-bin',
                                ...
                                (0, log_1.log)('debug', 'Corrected path also failed', {
                                    correctedPath,
                ...
                (0, log_1.log)('debug', 'Failed to require 7z-bin package', {
                    error: requireErr.message,
                    suggestion: 'Package may not be installed'
                });
            }
            ...
            // On non-macOS platforms, try 7z-bin first
            try {
                (0, log_1.log)('debug', 'Attempting to resolve 7z-bin package on non-macOS platform');
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const sevenBinPath = require('7z-bin');
                if (sevenBinPath) {
                    (0, log_1.log)('debug', 'Found 7z-bin package path', { path: sevenBinPath });
                    try {
                        ...
                        (0, log_1.log)('debug', 'Failed to stat 7z-bin package path', {
                            path: sevenBinPath,
                            error: statErr.message
                        });
                    }
            ...
            // Then try 7zip-bin package
            try {
                (0, log_1.log)('debug', 'Attempting to resolve 7zip-bin package on non-macOS platform');
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const sevenZipBin = require('7zip-bin');
                if (sevenZipBin) {
                    // 7zip-bin exports an object with path7za property, not a string
        ...
        const duration = Date.now() - startTime;
        (0, log_1.log)('warn', 'No bundled 7z tools found', {
            platform: process.platform,
            arch: process.arch,
            modulesBase,
        ...
        const duration = Date.now() - startTime;
        (0, log_1.log)('error', 'Error during bundled 7z tool resolution', {
            error: err.message,
            platform: process.platform,
            arch: process.arch,
            duration: `${duration}ms`,
