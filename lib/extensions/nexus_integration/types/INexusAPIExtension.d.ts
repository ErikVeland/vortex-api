...
export interface INexusAPIExtension {
    ...
    nexusSubmitFeedback?: (title: string, message: string, hash: string, feedbackFiles: string[], anonymous: boolean, callback: (err: Error | null, response?: IFeedbackResponse) => void) => void;
    nexusSubmitCollection?: (collectionInfo: ICollectionManifest, assetFilePath: string, collectionId: number, callback: (err: Error | null, response?: {
        id: number;
    } | {
        success: boolean;
        message?: string;
