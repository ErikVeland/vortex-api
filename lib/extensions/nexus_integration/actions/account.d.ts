import * as reduxAct from 'redux-act';
export declare const setUserAPIKey: reduxAct.ComplexActionCreator1<string, string, {}>;
export declare const clearOAuthCredentials: reduxAct.ComplexActionCreator1<null, null, {}>;
export declare const setOAuthCredentials: reduxAct.ComplexActionCreator3<string, string, string, {
    token: string;
    refreshToken: string;
    fingerprint: string;
}, {}>;
export declare const setForcedLogout: reduxAct.ComplexActionCreator1<boolean, boolean, {}>;
