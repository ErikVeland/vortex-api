...
function purgeDeployedFiles(basePath, files) {
    return (0, bluebird_migration_helpers_local_1.promiseMap)(files, file => {
...
function fallbackPurge(api, gameId) {
    const state = api.store.getState();
    if (gameId === undefined) {
        gameId = (0, activeGameId_1.activeGameId)(state);
    }
    ...
    if ((game === undefined)
        ...
        return Promise.reject(new CustomErrors_1.ProcessCanceled('game got disabled'));
    }
    const modPaths = game.getModPaths(gameDiscovery.path);
    const stagingPath = (0, selectors_2.installPathForGame)(state, gameId);
    const activator = (0, deploymentMethods_1.getCurrentActivator)(state, gameId, false);
    return (0, bluebird_migration_helpers_local_1.promiseEach)(Object.keys(modPaths), typeId => fallbackPurgeType(api, activator, gameId, typeId, modPaths[typeId], stagingPath))
        .then(() => undefined);
}
...
function saveActivation(gameId, modType, instance, gamePath, stagingPath, activation, activatorId) {
    ...
    return (activation.length === 0)
        ? fs.removeAsync(tagFilePath)
            .catch(() => undefined)
            .then(() => undefined)
        : Promise.resolve((0, fsAtomic_1.writeFileAtomic)(tagFilePath, dataJSON))
