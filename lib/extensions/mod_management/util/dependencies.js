...
function browseForDownload(api, url, instruction) {
    return new Promise((resolve, reject) => {
                ...
                lookupResult = api.emitAndAwait('browse-for-download', url, instruction, true)
                    ...
                    if (resultList[0].startsWith('err:')) {
                        ...
                        if (msg === 'skip') {
                            ...
                            return Promise.reject(new CustomErrors_1.UserCanceled(false));
                        }
        ...
        return resolve({
            url: () => doLookup().then(out => Promise.resolve(out === null || out === void 0 ? void 0 : out.url)),
...
function lookupDownloadHint(api, input) {
    ...
    if (input.mode === 'direct') {
        ...
        try {
            ...
            return Promise.reject(new CustomErrors_1.NotFound(`Invalid URL set for external dependency: "${(_b = input.url) !== null && _b !== void 0 ? _b : '<unset>'}"`));
        }
        ...
        try {
            ...
            return Promise.reject(new CustomErrors_1.NotFound(`Invalid URL set for external dependency: "${(_d = input.url) !== null && _d !== void 0 ? _d : '<unset>'}"`));
        }
        return browseForDownload(api, urlNorm, input.instructions)
            .then(result => {
            if (result === undefined) {
                return Promise.reject(new CustomErrors_1.NotFound('No download found browsing url'));
            }
            else {
            ...
            if (err instanceof CustomErrors_1.UserCanceled) {
                return Promise.reject(new CustomErrors_1.UserCanceled((_a = err.skipped) !== null && _a !== void 0 ? _a : true));
            }
            else {
        ...
        return Promise.reject(new CustomErrors_1.ProcessCanceled(input.instructions));
    }
}
...
function findDownloadByRef(reference, downloads) {
    ...
    try {
        ...
        const existing = Object.keys(downloads).filter((dlId) => {
            ...
            fileIdSet.add((_b = (_a = lookup === null || lookup === void 0 ? void 0 : lookup.fileId) === null || _a === void 0 ? void 0 : _a.toString) === null || _b === void 0 ? void 0 : _b.call(_a));
            ...
            nameSet.add((_c = download.modInfo) === null || _c === void 0 ? void 0 : _c.name);
            const identifiers = {
            ...
            return (0, testModReference_1.testRefByIdentifiers)(identifiers, reference) || (0, testModReference_1.default)(lookup, reference, undefined, fuzzy);
        })
...
function gatherDependenciesGraph(rule, api, gameMode, recommendations) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let urlFromHint;
        let lookupResults = [];
        const limit = new ConcurrencyLimiter_1.default(10);
        try {
            if (!download) {
 ...
 */
function gatherDependencies(rules, api, recommendations, progressCB) {
    const state = api.getState();
    const gameMode = (0, activeGameId_1.activeGameId)(state);
    ...
    return Promise.all(requirements.map((rule) => Promise.resolve(limit.do(() => gatherDependenciesGraph(rule, api, gameMode, recommendations)))
        .then((node) => {
        onProgress();
