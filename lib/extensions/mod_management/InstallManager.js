...
class InstallManager {
    ...
    pollPhaseSettlement(api, sourceModId, options) {
        ...
        return new bluebird_1.default((resolve) => {
            const poll = () => {
                ...
                const active = (_c = phaseState.activeByPhase.get(checkPhase)) !== null && _c !== void 0 ? _c : 0;
                const pending = ((_d = phaseState.pendingByPhase.get(checkPhase)) !== null && _d !== void 0 ? _d : []).length;
    ...
    scheduleDeployOnPhaseSettled(api, sourceModId, phase) {
        this.ensurePhaseState(sourceModId);
    ...
    maybeAdvancePhase(sourceModId, api) {
        ...
        while (state.downloadsFinished.has(curr)
            ...
            if (!state.deployedPhases.has(curr)) {
                (0, log_1.log)('debug', 'phase gating: waiting for current phase deployment', { sourceModId, currPhase: curr });
            ...
            if (nextIdx < finished.length) {
                ...
                if (state.downloadsFinished.has(curr)) {
                    (0, log_1.log)('debug', 'Advanced to new phase, scheduling deployment polling', { sourceModId, newPhase: curr });
                    // Schedule deployment polling for the newly allowed phase
                    if (api) {
                        this.scheduleDeployOnPhaseSettled(api, sourceModId, curr);
                    }
                    else {
                        (0, log_1.log)('warn', 'Cannot schedule deployment polling - API not provided to maybeAdvancePhase', { sourceModId, phase: curr });
                    }
                }
                continue;
            }
