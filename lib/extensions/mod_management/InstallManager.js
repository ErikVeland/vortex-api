...
exports.VARIANT_ACTION = exports.REPLACE_ACTION = exports.INSTALL_ACTION = exports.INI_TWEAKS_PATH = exports.ArchiveBrokenError = exports.DynamicDownloadConcurrencyLimiter = void 0;
/* eslint-disable */
...
const dependencyToModInfo_1 = require("./util/dependencyToModInfo");
...
exports.DynamicDownloadConcurrencyLimiter = DynamicDownloadConcurrencyLimiter;
...
const activeGameId_2 = require("../profile_management/activeGameId");
const selectors_3 = require("../download_management/selectors");
const selectors_4 = require("./selectors");
const selectors_5 = require("../gamemode_management/selectors");
...
function getContentPolicy(gameId) {
    // Placeholder for future game-specific overrides; can be extended via settings or game config
    switch (gameId) {
        case 'skyrimse':
            return Object.assign(Object.assign({}, DEFAULT_POLICY), { blockedExtensions: ['.dll'] });
        default:
...
class InstallManager {
    constructor(api, installPath) {
        ...
        this.mDependencyQueue = (0, util_1.makeQueue)();
        ...
        this.mMainInstallsLimit = new ConcurrencyLimiter_1.default(3);
        ...
        api.onAsync('install-from-dependencies', (dependentId, rules, recommended) => {
            var _a;
            const profile = (0, activeGameId_1.activeProfile)(api.getState());
            if (profile === undefined) {
                return Promise.reject(new CustomErrors_1.ProcessCanceled('No game active'));
            }
            ...
            if (recommended) {
                return this.withDependenciesContext('install-recommendations', () => this.installRecommendationsImpl(api, profile, profile.gameId, dependentId, (0, modName_1.default)(collection), filtered, instPath, true));
            }
            else {
                return this.withDependenciesContext('install-collections', () => this.installDependenciesImpl(api, profile, profile.gameId, dependentId, (0, modName_1.default)(collection), filtered, instPath, true));
            }
        });
        ...
        api.onAsync('reset-dependency-installs', () => {
            ...
            this.mDependencyDownloadsLimit = new DynamicDownloadConcurrencyLimiter(api);
            this.mDependencyInstallsLimit = new ConcurrencyLimiter_1.default(3);
            return Promise.resolve();
        });
    }
    /**
    ...
    forceCleanupStuckInstalls(api, maxAgeMinutes = 10) {
        ...
        this.mActiveInstalls.forEach((install, installId) => {
            ...
            if (age > maxAgeMs) {
                ...
                (0, log_1.log)('warn', 'InstallManager: Force cleaning stuck installation', {
                    installId,
                    modId: install.modId,
    ...
    simulate(api, gameId, archivePath, tempPath, extractList, unattended, installChoices, progress) {
        ...
        if (policy.blockedExtensions.includes(archiveExt)) {
            extractProm = Promise.reject(new ArchiveBrokenError('file type blocked by policy'));
        }
        else {
     ...
     *                                 the auto-detection.
     */
    install(archiveId, archivePath, downloadGameIds, api, info, processDependencies, enable, callback, forceGameId, fileList, unattended, forceInstaller, allowAutoDeploy) {
        const baseName = path.basename(archivePath, path.extname(archivePath)).trim() || 'EMPTY_NAME';
        const installId = (0, shortid_1.generate)();
        ...
        if (archiveId && archiveId !== null) {
            ...
            if (download && download.state !== 'finished') {
                ...
                (0, log_1.log)('warn', 'Attempted to install unfinished download', {
                    installId,
                    archiveId,
        ...
        this.mMainInstallsLimit.do(() => {
            return new Promise((resolve, reject) => {
                ...
                const currentProfile = (0, activeGameId_1.activeProfile)(api.store.getState());
                ...
                const installationPromise = (0, errorHandling_1.withContext)('Installing', baseName, () => ((forceGameId !== undefined)
                    ...
                    const games = (0, selectors_5.knownGames)(state);
                    ...
                    if (games.find(iter => iter.id === installGameId) === undefined) {
                        return Promise.reject(new CustomErrors_1.ProcessCanceled(`Game not supported "${installGameId}"`));
                    }
                    if (installGameId !== (currentProfile === null || currentProfile === void 0 ? void 0 : currentProfile.gameId)) {
                        const installProfileId = (0, activeGameId_2.lastActiveProfileForGame)(state, installGameId);
                        ...
                        try {
                            ...
                            (0, log_1.log)('info', 'activated target game for install', { installGameId, currentProfile: currentProfile === null || currentProfile === void 0 ? void 0 : currentProfile.id, installProfileId });
                        }
                        catch (e) {
                            (0, log_1.log)('warn', 'failed to emit activate-game event', { installGameId, error: e === null || e === void 0 ? void 0 : e.message });
                        }
                    }
                    ...
                    if (existingHash && existingSize) {
                        (0, log_1.log)('info', 'Using existing hash from download, skipping recalculation', {
                            archivePath: path.basename(archivePath),
                            hash: existingHash,
                            size: existingSize
                        });
                    ...
                    if (installGameId === 'site') {
                        ...
                        return api.emitAndAwait('install-extension-from-download', archiveId)
                            .then(() => Promise.reject(new CustomErrors_1.UserCanceled()));
                    }
                    const silent = unattended && allowAutoDeploy && enable;
                    installContext = new InstallContext_1.default(installGameId, api, silent);
                    installContext.startIndicator(baseName);
                    let dlGame = (0, storeHelper_1.getSafe)(fullInfo, ['download', 'game'], '');
                    if (Array.isArray(dlGame)) {
                        dlGame = dlGame[0];
                    }
                    // TODO: remove this hack once fileHashes is optimized
                    const createKey = (hash, size, gameId) => {
                        let lookupKey = `${hash}`;
                        if (size !== undefined) {
                            lookupKey += ':' + size;
                            if (gameId !== undefined) {
                    ...
                    const lookupKey = createKey(archiveMD5, archiveSize, dlGame);
                    const modInfo = (0, dependencyToModInfo_1.dependencyToModInfo)(fullInfo, dlGame);
                    return Promise.resolve([{ key: lookupKey, value: modInfo }]);
                })
                    .then((modInfo) => {
                    ...
                    return this.installInner(api, archivePath, tempPath, destinationPath, installGameId, installContext, undefined, forceInstaller, fullInfo.choices, fileList, unattended);
                })
                    .then(result => {
                    ...
                    .then((result) => this.processInstructions(api, installContext, archivePath, tempPath, destinationPath, installGameId, modId, result, fullInfo.choices, unattended))
                    .finally(() => {
                    ...
                    let prom = destinationPath !== undefined
                        ? fs.removeAsync(destinationPath)
                            .catch(err => err instanceof CustomErrors_1.UserCanceled ? null : Promise.reject(err))
                            .catch(innerErr => {
                                    ...
                                    actions: [
                                        ...
                                        { title: 'Delete & Redownload', action: dismiss => {
                                                ...
                                                api.events.emit('remove-download', archiveId, () => {
                                                    dismiss();
                                                    api.events.emit('start-download', download.urls, info.download, path.basename(archivePath), undefined, 'replace');
                                                });
                                                dismiss();
                                            } },
                                    ],
                    ...
                    if (installContext !== undefined) {
                        ...
                        (0, log_1.log)('debug', 'InstallManager: Cleaning up installation in finally block', {
                            installId,
                            modId: modId || 'unknown',
                            indicatorId: installContext === null || installContext === void 0 ? void 0 : installContext['mIndicatorId'],
                        ...
                        try {
                            ...
                            (0, log_1.log)('debug', 'InstallManager: stopIndicator completed successfully', {
                                installId,
                                modId: modId || 'unknown'
                            });
                        }
                        ...
                        (0, log_1.log)('warn', 'InstallManager: installContext is undefined in finally block', {
                            installId,
                            modId: modId || 'unknown'
                        });
                    }
                ...
                installationPromise
                    ...
                    if (this.mActiveInstalls.has(installId)) {
                        ...
                        if (installContext !== undefined) {
                            (0, log_1.log)('warn', 'InstallManager: Manually triggering notification cleanup for orphaned installation', {
                                installId,
                                modId,
                                indicatorId: installContext === null || installContext === void 0 ? void 0 : installContext['mIndicatorId']
                            });
    ...
    installDependencies(api, profile, gameId, modId, allowAutoDeploy) {
        var _a;
        const state = api.store.getState();
        const mod = (_a = state.persistent.mods[gameId]) === null || _a === void 0 ? void 0 : _a[modId];
        if (mod === undefined) {
            return Promise.reject(new CustomErrors_1.ProcessCanceled(`Invalid mod specified "${modId}"`));
        }
        this.repairRules(api, mod, gameId);
        const installPath = this.mGetInstallPath(gameId);
        (0, log_1.log)('info', 'start installing dependencies');
        api.store.dispatch((0, actions_1.startActivity)('installing_dependencies', mod.id));
        return this.withDependenciesContext('install-dependencies', () => this.augmentRules(api, gameId, mod)
            .then(rules => this.installDependenciesImpl(api, profile, gameId, mod.id, (0, modName_1.default)(mod), rules, installPath, allowAutoDeploy))
            .finally(() => {
            (0, log_1.log)('info', 'done installing dependencies');
            api.store.dispatch((0, actions_1.stopActivity)('installing_dependencies', mod.id));
        }));
    }
    installRecommendations(api, profile, gameId, modId) {
        ...
        this.repairRules(api, mod, gameId);
        const installPath = this.mGetInstallPath(gameId);
        (0, log_1.log)('info', 'start installing recommendations');
        api.store.dispatch((0, actions_1.startActivity)('installing_dependencies', mod.id));
        return this.withDependenciesContext('install-recommendations', () => this.augmentRules(api, gameId, mod)
            .then(rules => this.installRecommendationsImpl(api, profile, gameId, mod.id, (0, modName_1.default)(mod), rules, installPath, false))
            .finally(() => {
            (0, log_1.log)('info', 'done installing recommendations');
            api.store.dispatch((0, actions_1.stopActivity)('installing_dependencies', mod.id));
        }));
    }
    augmentRules(api, gameId, mod) {
        var _a;
        const rules = ((_a = mod.rules) !== null && _a !== void 0 ? _a : []).slice();
        return Promise.resolve(rules); // This should be calling the api.lookupmetadata function but it's not reliable
    }
    ...
    setModSize(api, modId, gameId) {
        var _a;
        const state = api.getState();
        const stagingFolder = (0, selectors_4.installPathForGame)(state, gameId);
        const mod = (_a = state.persistent.mods[gameId]) === null || _a === void 0 ? void 0 : _a[modId];
     ...
     */
    cleanupPendingInstalls(sourceModId) {
        ...
        (0, log_1.log)('debug', 'Cleaned up pending and active installations', {
            sourceModId,
     ...
     */
    queueInstallation(api, dep, downloadId, gameId, sourceModId, recommended) {
        ...
        this.mDependencyInstallsLimit.do(() => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (((_a = downloads[downloadId]) === null || _a === void 0 ? void 0 : _a.state) !== 'finished') {
                ...
                if (existingMod) {
                    (0, log_1.log)('info', 'Mod already installed, skipping', { modId: existingMod.id });
                    return;
                }
                (0, log_1.log)('info', 'Processing queued installation in parallel', {
                    downloadId,
                    dependency: (0, modName_1.renderModReference)(currentDep.reference)
                });
                const sourceMod = api.getState().persistent.mods[gameId][sourceModId];
                const modId = yield this.withInstructions(api, (0, modName_1.default)(sourceMod), (0, modName_1.renderModReference)(currentDep.reference), (_c = (_b = currentDep.reference) === null || _b === void 0 ? void 0 : _b.tag) !== null && _c !== void 0 ? _c : downloadId, (_d = currentDep.extra) === null || _d === void 0 ? void 0 : _d['instructions'], recommended, () => this.installModAsync(currentDep.reference, api, downloadId, { choices: currentDep.installerChoices, patches: currentDep.patches }, currentDep.fileList, gameId, true));
                if (modId) {
                    this.mActiveInstalls.delete(installKey);
                    // Apply any extra attributes
                    this.applyExtraFromRule(api, gameId, modId, Object.assign(Object.assign({}, currentDep.extra), { fileList: (_e = currentDep.fileList) !== null && _e !== void 0 ? _e : (_f = currentDep.extra) === null || _f === void 0 ? void 0 : _f.fileList, installerChoices: currentDep.installerChoices, patches: (_g = currentDep.patches) !== null && _g !== void 0 ? _g : (_h = currentDep.extra) === null || _h === void 0 ? void 0 : _h.patches }));
                ...
                this.mActiveInstalls.delete(installKey);
                (0, log_1.log)('warn', 'Failed to install dependency in parallel', {
                    downloadId,
                    error: err.message,
            ...
            (0, log_1.log)('error', 'Critical error in parallel installation', {
                downloadId,
                error: err.message,
                dependency: (0, modName_1.renderModReference)(dep.reference)
            });
     ...
     */
    installInner(api, archivePath, tempPath, destinationPath, gameId, installContext, installationZip, forceInstaller, installChoices, extractList, unattended) {
        const fileList = [];
        ...
        return extractProm
            ...
            .catch(err => {
            if (err instanceof ArchiveBrokenError) {
            ...
            if ([stagingDirectory_1.STAGING_DIR_TAG, downloadDirectory_1.DOWNLOADS_DIR_TAG].indexOf(path.basename(archivePath)) !== -1) {
                ...
                return api.showDialog('question', 'Not a mod', {
                    ...
                    { label: 'Ok' },
                ]).then(() => Promise.reject(new CustomErrors_1.ProcessCanceled('Not a mod')));
            }
            ...
            fileList.forEach(rel => {
                ...
                if ((_a = policy.blockedExtensions) === null || _a === void 0 ? void 0 : _a.includes(ext)) {
                    foundBlocked.push(rel);
                }
                if ((_b = policy.confirmExtensions) === null || _b === void 0 ? void 0 : _b.includes(ext)) {
            ...
            if (foundBlocked.length > 0) {
                ...
                yield api.showDialog('error', title, { bbcode }, [{ label: 'Ok' }]);
            ...
            if (foundExecutables.length > 0) {
                ...
                const result = yield api.showDialog('question', title, { bbcode }, [
                    { label: 'Cancel' },
                    { label: 'Continue' },
                ]);
                if ((result === null || result === void 0 ? void 0 : result.action) === 'Cancel') {
    ...
    determineModType(gameId, installInstructions, retryCount = 0) {
        (0, log_1.log)('info', 'determine mod type', { gameId, retryCount });
        const game = (0, getGame_1.getGame)(gameId);
        if (game === undefined) {
        ...
        if (modTypes.length === 0 && retryCount < 3) {
            (0, log_1.log)('info', 'no mod types available, waiting for extensions to load', { gameId, retryCount });
            return (0, bluebird_migration_helpers_local_1.promiseDelay)(1000).then(() => this.determineModType(gameId, installInstructions, retryCount + 1));
        }
        const sorted = modTypes.sort((lhs, rhs) => lhs.priority - rhs.priority);
        let found = false;
        return (0, bluebird_migration_helpers_local_1.promiseMapSeries)(sorted, (type) => {
            if (found) {
            ...
            if (result === '' && retryCount < 3 && modTypes.length > 0) {
                (0, log_1.log)('info', 'no mod type match found, retrying', { gameId, retryCount, availableTypes: modTypes.map(t => t.typeId) });
                return (0, bluebird_migration_helpers_local_1.promiseDelay)(500).then(() => this.determineModType(gameId, installInstructions, retryCount + 1));
            }
            return result;
        });
    }
    ...
    validateInstructions(instructions) {
        ...
        const invalidDestinationErrors = instructions.filter(instr => {
            if (!!instr.destination) {
                ...
                const sanitized = (0, platform_1.isWindows)()
    ...
    processMKDir(instructions, destinationPath) {
        return (0, bluebird_migration_helpers_local_1.promiseEach)(instructions, instruction => fs.ensureDirAsync(path.join(destinationPath, instruction.destination)))
            .then(() => undefined);
    }
    processGenerateFiles(generatefile, destinationPath) {
        return (0, bluebird_migration_helpers_local_1.promiseEach)(generatefile, gen => {
    ...
    processSubmodule(api, installContext, submodule, destinationPath, gameId, modId, choices, unattended) {
        return (0, bluebird_migration_helpers_local_1.promiseEach)(submodule, mod => {
            ...
            return this.installInner(api, mod.path, tempPath, destinationPath, gameId, subContext, undefined, undefined, choices, undefined, unattended)
    ...
    processIniEdits(api, iniEdits, destinationPath, gameId, modId) {
        ...
        return fs.ensureDirAsync(path.join(destinationPath, exports.INI_TWEAKS_PATH))
            .then(() => (0, bluebird_migration_helpers_local_1.promiseMap)(Object.keys(byDest), destination => {
    ...
    processInstructions(api, installContext, archivePath, tempPath, destinationPath, gameId, modId, result, choices, unattended) {
        ...
        if ((result.instructions === undefined) ||
            ...
            return Promise.reject(new CustomErrors_1.ProcessCanceled('Empty archive or no options selected'));
        }
        ...
        if (invalidInstructions.length > 0) {
            ...
            return Promise.reject(new CustomErrors_1.ProcessCanceled('Invalid installer instructions'));
        }
        ...
        if (instructionGroups.error.length > 0) {
            ...
            if (fatal !== undefined) {
                return Promise.reject(new CustomErrors_1.ProcessCanceled('Installer script failed'));
            }
        }
    ...
    queryProfileCount(store) {
        ...
        const profiles = (0, selectors_2.gameProfiles)(state);
    ...
    userVersionChoice(oldMod, store) {
        ...
        const handleAction = (action, remember) => {
            ...
            if (action === 'Cancel') {
                ...
                return Promise.resolve(exports.REPLACE_ACTION);
            }
            else if (action === exports.INSTALL_ACTION) {
                return Promise.resolve(exports.INSTALL_ACTION);
            }
        };
        ...
        return rememberAction ? Promise.resolve(rememberAction) : (totalProfiles === 1)
            ? Promise.resolve(exports.REPLACE_ACTION)
    ...
    queryUserReplace(api, modIds, gameId, installOptions) {
        return new Promise((resolve, reject) => {
            ...
            if (choices === undefined) {
                choices = isDependency ? Promise.resolve({ action: 'replace', remember: true }) : queryDialog();
            }
            choices
                .then((result) => {
                var _a, _b, _c;
    ...
    downloadURL(api, lookupResult, wasCanceled, referenceTag, campaign, fileName) {
        ...
        return call(lookupResult.sourceURI).then(res => resolvedSource = res)
            ...
            if (!api.events.emit('start-download', [url.format(parsedUrl)], {
                game: (0, convertGameId_1.convertGameIdReverse)((0, selectors_5.knownGames)(api.store.getState()), lookupResult.domainName),
    ...
    downloadMatching(api, lookupResult, pattern, referenceTag, wasCanceled, campaign, fileName) {
        ...
        const gameId = (0, convertGameId_1.convertGameIdReverse)((0, selectors_5.knownGames)(api.getState()), lookupResult.domainName || lookupResult.gameId);
        return api.emitAndAwait('start-download-update', lookupResult.source, gameId, modId, fileId, pattern, campaign, referenceTag)
            ...
            if ((results === undefined) || (results.length === 0)) {
                return Promise.reject(new CustomErrors_1.NotFound(`source not supported "${lookupResult.source}"`));
            }
            else {
                if (!(0, util_1.truthy)(results[0])) {
                    return Promise.reject(new CustomErrors_1.ProcessCanceled('Download failed', { alreadyReported: true }));
                }
                else {
    ...
    doInstallDependenciesPhase(api, dependencies, gameId, sourceModId, recommended, downloadAndInstall, abort) {
        const res = (0, bluebird_migration_helpers_local_1.promiseMap)(dependencies, (dep) => __awaiter(this, void 0, void 0, function* () {
            ...
            return downloadAndInstall(dep)
                ...
                this.applyExtraFromRule(api, gameId, modId, Object.assign(Object.assign({}, dep.extra), { fileList: (_a = dep.fileList) !== null && _a !== void 0 ? _a : (_b = dep.extra) === null || _b === void 0 ? void 0 : _b.fileList, installerChoices: dep.installerChoices, patches: (_c = dep.patches) !== null && _c !== void 0 ? _c : (_d = dep.extra) === null || _d === void 0 ? void 0 : _d.patches }));
                ...
                if (err instanceof CustomErrors_1.ProcessCanceled) {
                    ...
                    const refName = (0, modName_1.renderModReference)(dep.reference, undefined);
                    api.showErrorNotification('Failed to install dependency', '{{errorMessage}}\nA common cause for issues here is that the file may no longer '
                ...
                .catch(err => {
                if (err instanceof DownloadManager_1.DownloadIsHTML) {
                    const refName = (0, modName_1.renderModReference)(dep.reference, undefined);
                    api.showErrorNotification('Failed to install dependency', 'The direct download URL for this file is not valid or didn\'t lead to a file. '
                ...
                .catch(err => {
                if (err instanceof CustomErrors_1.NotFound) {
                    const refName = (0, modName_1.renderModReference)(dep.reference, undefined);
                ...
                if (err instanceof CustomErrors_1.UserCanceled) {
                    ...
                    api.showErrorNotification('Download failed', 'The download ended prematurely or was corrupted. You\'ll have to restart it.', {
                        allowReport: false,
                    });
                }
                    ...
                    api.showErrorNotification('Failed to install dependency', 'The URL protocol used in the dependency is not supported, '
            ...
            .then(deps => deps.filter(dep => dep !== undefined));
    ...
    doInstallDependencies(api, gameId, sourceModId, dependencies, recommended, silent) {
        ...
        const stagingPath = (0, selectors_4.installPathForGame)(state, gameId);
        ...
        const queueDownload = (dep) => {
            return this.mDependencyDownloadsLimit.do(() => {
                ...
                return abort.signal.aborted
                    ? Promise.reject(new CustomErrors_1.UserCanceled(false))
                    : this.downloadDependencyAsync(dep.reference, api, dep.lookupResults[0].value, () => abort.signal.aborted, (_a = dep.extra) === null || _a === void 0 ? void 0 : _a.fileName)
                        ...
                        if (err instanceof DownloadManager_1.AlreadyDownloaded) {
                            if (err.downloadId !== undefined) {
                                ...
                                if (downloadId) {
                                    (0, log_1.log)('info', 'File already downloaded, found existing download', { downloadId, fileName: err.fileName });
                                    ...
                                    (0, log_1.log)('warn', 'File already exists but no download record found, treating as completed', {
                                        fileName: err.fileName,
                                        reference: (_a = dep.reference) === null || _a === void 0 ? void 0 : _a.tag
                                    });
                                    // Create a placeholder download ID or return a special value that indicates file exists
                                    return Promise.resolve(err.downloadId || 'file-exists-' + Date.now());
                                }
                            }
                        }
                        // Check if this is a network error that might have caused the download to be paused
                        const isNetworkError = ((_b = err.message) === null || _b === void 0 ? void 0 : _b.includes('socket hang up'))
                            || ((_c = err.message) === null || _c === void 0 ? void 0 : _c.includes('ECONNRESET'))
                            || ((_d = err.message) === null || _d === void 0 ? void 0 : _d.includes('ETIMEDOUT'))
                            || err.code === 'ECONNRESET'
                            || err.code === 'ETIMEDOUT';
                        // Check if this is a "File already downloaded" error (for cases where we get a generic error message)
                        const isAlreadyDownloaded = ((_e = err.message) === null || _e === void 0 ? void 0 : _e.includes('File already downloaded'))
                            || ((_f = err.message) === null || _f === void 0 ? void 0 : _f.includes('already downloaded'));
                        if (isAlreadyDownloaded) {
                            ...
                            if (downloadId) {
                                ...
                                (0, log_1.log)('warn', 'File already downloaded but no download record found, treating as completed', {
                                    reference: (_g = dep.reference) === null || _g === void 0 ? void 0 : _g.tag
                                });
                                return Promise.resolve('file-exists-' + Date.now());
                            }
                        }
                        if (isNetworkError) {
        ...
        const resumeDownload = (dep) => {
            ...
            return this.mDependencyDownloadsLimit.do(() => abort.signal.aborted
                ...
                : new Promise((resolve, reject) => {
                    ...
                    if (!currentDownload) {
                        (0, log_1.log)('warn', 'Download not found when trying to resume', { downloadId: dep.download });
                        return reject(new CustomErrors_1.NotFound(`Download ${dep.download} not found`));
                    }
                    if (currentDownload.state === 'finished') {
                        (0, log_1.log)('info', 'Download already finished, no need to resume', { downloadId: dep.download });
                        return resolve(dep.download);
                    }
                    if (currentDownload.state !== 'paused') {
                        (0, log_1.log)('info', 'Download not in paused state, current state:', {
                            downloadId: dep.download,
                            state: currentDownload.state
                        });
                        return resolve(dep.download);
                    }
                    (0, log_1.log)('info', 'Resuming paused download', {
                        downloadId: dep.download,
                        tag: (_a = dep.reference) === null || _a === void 0 ? void 0 : _a.tag
                    });
                    api.events.emit('resume-download', dep.download, (err) => {
                        var _a, _b;
                        if (err !== null) {
                            ...
                            if (((_a = err.message) === null || _a === void 0 ? void 0 : _a.includes('File already downloaded')) || ((_b = err.message) === null || _b === void 0 ? void 0 : _b.includes('already downloaded'))) {
                                (0, log_1.log)('info', 'Download already completed during resume attempt', { downloadId: dep.download });
                                return resolve(dep.download);
                            }
                            reject(err);
                        }
        ...
        const doDownload = (dep) => {
            ...
            if ((dep.download === undefined) || (downloads[dep.download] === undefined)) {
                if (((_a = dep.extra) === null || _a === void 0 ? void 0 : _a.localPath) !== undefined) {
                    ...
                    const downloadPath = (0, selectors_3.downloadPathForGame)(state, gameId);
                    ...
                    dlPromise = ((_d = (_c = (_b = dep.lookupResults[0]) === null || _b === void 0 ? void 0 : _b.value) === null || _c === void 0 ? void 0 : _c.sourceURI) !== null && _d !== void 0 ? _d : '') === ''
                        ? Promise.reject(new CustomErrors_1.ProcessCanceled('Failed to determine download url'))
                        : queueDownload(dep);
                }
            }
            else if (dep.download === null) {
                dlPromise = Promise.reject(new CustomErrors_1.ProcessCanceled('Failed to determine download url'));
            }
            ...
            return dlPromise
                ...
                if (err instanceof DownloadManager_1.AlreadyDownloaded) {
                    ...
                    return Promise.reject(new CustomErrors_1.NotFound(`download for ${(0, modName_1.renderModReference)(dep.reference)}`));
                }
                else {
                    return Promise.reject(err);
                }
                ...
                if (downloads[downloadId].state !== 'finished') {
                    ...
                    return Promise.reject(new CustomErrors_1.UserCanceled(true));
                }
                if ((dep.reference.tag !== undefined)
                ...
                if ((dep.mod === undefined)
                    && (((_d = dep.reference) === null || _d === void 0 ? void 0 : _d.versionMatch) !== undefined)
                    && !(0, testModReference_1.isFuzzyVersion)(dep.reference.versionMatch)
                    ...
                    queryWrongMD5 = () => api.showDialog('question', 'Unrecognized file {{reference}}', {
                        ...
                        parameters: {
                            ...
                            reference: (0, modName_1.renderModReference)(dep.reference, dep.mod),
                        },
                    }, [
                        { label: 'Retry' },
                        ...
                        ? Promise.reject(new CustomErrors_1.ProcessCanceled('retry invalid download'))
                        : Promise.resolve());
                }
                return (dep.mod == null)
                    ? queryWrongMD5()
                        ...
                        if (collectionsInstallWhileDownloading && !!dep) {
                            ...
                            (0, log_1.log)('info', 'Queued installation while download continues', {
                                downloadId,
                                dependency: (0, modName_1.renderModReference)(dep.reference)
                            });
                            return Promise.resolve(downloadId);
                            ...
                            (0, log_1.log)('info', 'Deferring installation until download is complete', {
                                downloadId,
                                dependency: (0, modName_1.renderModReference)(dep.reference)
                            });
                            return Promise.resolve(downloadId);
                        ...
                        if (download !== undefined) {
                            ...
                            const games = (0, selectors_5.knownGames)(newState);
                            ...
                            const fullPath = path.join((0, selectors_3.downloadPathForGame)(newState, pathGameId), download.localPath);
        ...
        const findDownloadId = (dep) => {
            return Object.keys(downloads).find(dlId => { var _a, _b; return ((_a = downloads[dlId].modInfo) === null || _a === void 0 ? void 0 : _a.referenceTag) === ((_b = dep === null || dep === void 0 ? void 0 : dep.reference) === null || _b === void 0 ? void 0 : _b.tag); });
        };
        const res = (0, bluebird_migration_helpers_local_1.promiseReduce)(phaseList, (prev, depList) => {
            if (depList.length === 0) {
                return Promise.resolve(prev);
            }
            return this.doInstallDependenciesPhase(api, depList, gameId, sourceModId, recommended, doDownload, abort)
                ...
                if (collectionsInstallWhileDownloading) {
                    ...
                    return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                        const sorted = ([...updated]).sort((a, b) => { var _a, _b, _c, _d; return ((_b = (_a = a === null || a === void 0 ? void 0 : a.reference) === null || _a === void 0 ? void 0 : _a.fileSize) !== null && _b !== void 0 ? _b : 0) - ((_d = (_c = b === null || b === void 0 ? void 0 : b.reference) === null || _c === void 0 ? void 0 : _c.fileSize) !== null && _d !== void 0 ? _d : 0); }).filter(dep => !!dep);
                        try {
                            (0, log_1.log)('info', 'Processing deferred installations for completed downloads', {
                                count: sorted.length
                            });
                            // Install all dependencies that have completed downloads
                            const installResults = yield Promise.all(sorted.map((dep) => __awaiter(this, void 0, void 0, function* () {
                                var _a, _b;
                                // Skip if mod is already installed
                                if (dep.mod !== undefined) {
                                    (0, log_1.log)('debug', 'Dependency already installed, skipping', {
                                        modId: dep.mod.id,
                                        dependency: (0, modName_1.renderModReference)(dep.reference)
                                    });
                                    return dep;
                                ...
                                if (downloadId) {
                                    const currentDownloads = api.getState().persistent.downloads.files;
                                    if (((_a = currentDownloads[downloadId]) === null || _a === void 0 ? void 0 : _a.state) === 'finished') {
                                        // For deferred installations, we can install immediately since downloads are complete
                                        try {
                                            const modId = yield installDownload(dep, downloadId);
                                            if (modId) {
                                                (0, log_1.log)('info', 'Successfully installed deferred dependency', {
                                                    downloadId,
                                                    modId,
                                                    dependency: (0, modName_1.renderModReference)(dep.reference)
                                                });
                                                ...
                                                (0, log_1.log)('warn', 'Installation returned null modId', {
                                                    downloadId,
                                                    dependency: (0, modName_1.renderModReference)(dep.reference)
                                                });
                                                return dep;
                                            ...
                                            (0, log_1.log)('warn', 'Failed to install deferred dependency', {
                                                downloadId,
                                                error: err.message,
                                                dependency: (0, modName_1.renderModReference)(dep.reference)
                                            });
                                        ...
                                        (0, log_1.log)('info', 'Download not finished, skipping installation', {
                                            downloadId,
                                            state: (_b = currentDownloads[downloadId]) === null || _b === void 0 ? void 0 : _b.state,
                                            dependency: (0, modName_1.renderModReference)(dep.reference)
                                        });
                                    ...
                                    (0, log_1.log)('debug', 'No download ID found for dependency', {
                                        dependency: (0, modName_1.renderModReference)(dep.reference)
                                    });
                                    return dep;
                                }
                            ...
                            (0, log_1.log)('error', 'Error processing deferred installations', { error: err.message });
                            return reject(err);
                        }
                    }));
                }
                ...
                .then((updated) => {
                // Find the index of the current depList in phaseList
                // const idx = phaseList.indexOf(depList);
                if (phaseList.indexOf(depList) === phaseList.length - 1) {
                    return Promise.resolve(updated);
                }
                return (0, util_1.toPromise)(cb => api.events.emit('deploy-mods', cb))
                    .then(() => Promise.resolve(updated));
            })
                .then((updated) => Promise.resolve([].concat(prev, updated)));
        }, []);
    ...
    installDependenciesImpl(api, profile, gameId, modId, name, rules, installPath, silent) {
        const filteredRules = (rules !== null && rules !== void 0 ? rules : []).filter((rule) => ['recommends', 'requires'].includes(rule.type)
            && !rule.ignored);
    ...
    installRecommendationsImpl(api, profile, gameId, modId, name, rules, installPath, silent) {
        ...
        return (0, dependencies_1.default)(filteredRules, api, true, undefined)
            ...
            const remember = context.get('remember', null);
            let queryProm = Promise.resolve(success);
            if (!silent || (error.length > 0)) {
    ...
    installModAsync(requirement, api, downloadId, modInfo, fileList, forceGameId, silent) {
        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            const state = api.store.getState();
            ...
            let fullPath = path.join((0, selectors_3.downloadPathForGame)(state, pathGameId), download.localPath);
            ...
            if (convertedGameId && convertedGameId !== downloadGame[0]) {
                try {
                    ...
                    yield fs.statAsync(fullPath).catch(() => __awaiter(this, void 0, void 0, function* () {
                        ...
                        const originalPath = path.join((0, selectors_3.downloadPathForGame)(state, downloadGame[0]), download.localPath);
                        try {
    ...
    fixDestination(source, destination) {
    ...
    transferFile(source, destination, move) {
    ...
    extractArchive(api, archivePath, tempPath, destinationPath, copies, gameId) {
        let normalize;
        const dlPath = (0, selectors_3.downloadPathForGame)(api.getState(), gameId);
        const missingFiles = [];
        return fs.ensureDirAsync(destinationPath)
            .then(() => (0, getNormalizeFunc_1.default)(destinationPath))
            .then((normalizeFunc) => {
            normalize = normalizeFunc;
        })
            .then(() => {
            const sourceMap = copies.reduce((prev, copy) => {
                (0, util_1.setdefault)(prev, copy.source, [])
                    .push({ dest: copy.destination, section: copy.section });
                return prev;
            }, {});
            return (0, bluebird_migration_helpers_local_1.promiseMap)(Object.keys(sourceMap), srcRel => {
                ...
                return (0, bluebird_migration_helpers_local_1.promiseMapSeries)(destinations, (dest) => {
                    // Find the index of this destination in the destinations array
                    ...
                    return ((dest.section === 'download')
                        ...
                        if ((dest.section === 'download') && transferred) {
                            ...
                            return fs.statAsync(destPath)
                                .then(stat => {
                                fileSize = stat.size;
                                api.store.dispatch((0, actions_1.addLocalDownload)(archiveId, gameId, dest.dest, fileSize));
                                // Calculate MD5 asynchronously to avoid blocking
                                setImmediate(() => {
                                    (0, checksum_1.fileMD5)(destPath)
                                        .then(md5 => {
                                        api.store.dispatch((0, actions_1.setDownloadHashByFile)(dest.dest, md5, fileSize));
                                    })
                                        .catch(err => {
                                        (0, log_1.log)('warn', 'Failed to calculate MD5 for downloaded file', {
                                            file: dest.dest,
                                            error: err.message
                                        });
                                    });
            ...
            if (missingFiles.length > 0) {
                api.showErrorNotification(api.translate('Invalid installer'), api.translate('The installer in "{{name}}" tried to install files that were '
                    ...
                    + 'fine with NMM. The mod will likely work, at least partially.', { replace: { name: path.basename(archivePath) } })
