...
class InstallManager {
    ...
    simulate(api, gameId, archivePath, tempPath, extractList, unattended, installChoices, progress) {
        ...
        if (policy.blockedExtensions.includes(archiveExt)) {
            extractProm = bluebird_1.default.reject(new ArchiveBrokenError('file type blocked by policy'));
        }
        else {
            extractProm = bluebird_1.default.resolve()
                .then(() => {
        ...
        this.mMainInstallsLimit.do(() => {
            return new Promise((resolve, reject) => {
                ...
                const installationPromise = (0, errorHandling_1.withContext)('Installing', baseName, () => ((forceGameId !== undefined)
                    ...
                    installContext = new InstallContext_1.default(gameId, api, silent);
                    ...
                    return bluebird_1.default.resolve([{ key: lookupKey, value: modInfo }]);
                })
                    .then((modInfo) => {
    ...
    augmentRules(api, gameId, mod) {
        ...
        return bluebird_1.default.resolve(rules); // This should be calling the api.lookupmetadata function but it's not reliable
    }
    ...
    determineModType(gameId, installInstructions, retryCount = 0) {
        ...
        if (modTypes.length === 0 && retryCount < 3) {
            ...
            return bluebird_1.default.delay(1000).then(() => this.determineModType(gameId, installInstructions, retryCount + 1));
        }
        ...
        return bluebird_1.default.mapSeries(sorted, (type) => {
            ...
            if (result === '' && retryCount < 3 && modTypes.length > 0) {
                ...
                return bluebird_1.default.delay(500).then(() => this.determineModType(gameId, installInstructions, retryCount + 1));
            }
            return result;
        });
    }
    ...
    doInstallDependencies(api, gameId, sourceModId, dependencies, recommended, silent) {
                    ...
                    : this.downloadDependencyAsync(dep.reference, api, dep.lookupResults[0].value, () => abort.signal.aborted, (_a = dep.extra) === null || _a === void 0 ? void 0 : _a.fileName)
                        ...
                        if (err instanceof DownloadManager_1.AlreadyDownloaded) {
                            if (err.downloadId !== undefined) {
                                ...
                                if (downloadId) {
                                    ...
                                    return bluebird_1.default.resolve(err.downloadId || 'file-exists-' + Date.now());
                                }
                            }
                        }
                        ...
                        if (isAlreadyDownloaded) {
                            ...
                            if (downloadId) {
                                ...
                                return bluebird_1.default.resolve('file-exists-' + Date.now());
                            }
                        }
                        if (isNetworkError) {
        ...
        const doDownload = (dep) => {
            ...
            return dlPromise
                ...
                let queryWrongMD5 = () => bluebird_1.default.resolve();
                if ((dep.mod === undefined)
                    ...
                    queryWrongMD5 = () => api.showDialog('question', 'Unrecognized file {{reference}}', {
                        ...
                        ? bluebird_1.default.reject(new CustomErrors_1.ProcessCanceled('retry invalid download'))
                        : bluebird_1.default.resolve());
                }
                return (dep.mod == null)
                    ? queryWrongMD5()
                    ...
                    : bluebird_1.default.resolve(dep.mod.id);
            });
        };
        const phases = {};
        ...
        const res = bluebird_1.default.reduce(phaseList, (prev, depList, idx) => {
            ...
            return this.doInstallDependenciesPhase(api, depList, gameId, sourceModId, recommended, doDownload, abort)
                ...
                if (idx === phaseList.length - 1) {
                    return bluebird_1.default.resolve(updated);
                }
    ...
    fixDestination(source, destination) {
        ...
        return fs.statAsync(source)
            ...
            ? bluebird_1.default.resolve(destination)
            : fs.statAsync(destination)
                .then(destStat => destStat.isDirectory()
                ? path.join(destination, path.basename(source))
                : destination))
            .catch(() => bluebird_1.default.resolve(destination));
    }
    ...
    extractArchive(api, archivePath, tempPath, destinationPath, copies, gameId) {
        let normalize;
        const dlPath = (0, selectors_1.downloadPathForGame)(api.getState(), gameId);
        const missingFiles = [];
        return fs.ensureDirAsync(destinationPath)
            ...
            return bluebird_1.default.map(Object.keys(sourceMap), srcRel => {
                ...
                return bluebird_1.default.mapSeries(destinations, (dest, idx, len) => {
