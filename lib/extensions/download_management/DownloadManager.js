 ...
 */
class DownloadWorker {
    constructor(job, progressCB, finishCB, headersCB, userAgent, throttle, getAgent) {
        this.mBuffers = [];
        this.mDataHistory = [];
        this.mEnded = false;
        this.mWriting = false;
            ...
            try {
                ...
                this.mRequest = lib.request(requestOptions, (res) => {
                    ...
                    try {
                        ...
                        (0, log_1.log)('warn', 'URL encoding failed, using original', { url: jobUrl, error: err.message });
                        recodedURI = jobUrl;
                    }
        ...
        this.abort = (paused) => {
            ...
            if (this.mURLResolve !== undefined) {
                // Native Promises don't have a cancel method, so we just set it to undefined
        ...
        this.handleComplete = (str) => {
            ...
            this.writeBuffer(str)
                ...
                .catch(err => { if (err instanceof CustomErrors_1.UserCanceled)
        ...
        this.handleResponse = (response, jobUrl) => {
            ...
            if (response.statusCode >= 300) {
                if (([301, 302, 303, 307, 308].includes(response.statusCode))
                    ...
                    const newUrl = url.resolve(jobUrl, response.headers['location']);
                ...
                if (chunkable) {
                    ...
                    if (((_a = sizeMatch === null || sizeMatch === void 0 ? void 0 : sizeMatch.length) !== null && _a !== void 0 ? _a : 0) > 1) {
                        ...
                        if (!isNaN(start)) {
                            ...
                            if (delta !== 0) {
                                (0, log_1.log)('debug', 'aligning job offset with server content-range start', {
        ...
        this.writeBuffer = (str) => {
            ...
            try {
                ...
                return (0, bluebird_migration_helpers_local_1.promiseMapSeries)(bufs, buf => this.doWriteBuffer(buf))
                    .then(() => {
                    var _a;
        ...
        this.mURLResolve = Promise.resolve().then(() => job.url())
            .then(jobUrl => {
            this.mUrl = jobUrl;
...
class DownloadManager {
     ...
     */
    constructor(downloadPath, maxWorkers, maxChunks, speedCB, userAgent, protocolHandlers, maxBandwidth) {
        this.mBusyWorkers = {};
        this.mSlowWorkers = {};
        ...
        this.enqueue = (id, urls, fileName, progressCB, destinationPath, options) => {
            ...
            try {
                ...
                return Promise.reject(new CustomErrors_1.ProcessCanceled(`failed to parse url "${baseUrl}"`));
            }
        ...
        this.resolveUrls = (urls, name, friendlyName) => {
            ...
            return () => {
                if (cache === undefined) {
                    ...
                    cache = (0, bluebird_migration_helpers_local_1.promiseReduce)(urls, (prev, iter) => {
                        return this.resolveUrl(iter, name, friendlyName)
                            ...
                            .catch(err => err instanceof Error ? Promise.resolve(Object.assign(Object.assign({}, prev), { error: err })) : Promise.reject(err));
        ...
        this.startJob = (download, job) => {
            if (download.assemblerProm === undefined) {
                download.assemblerProm = FileAssembler_1.default.create(download.tempName)
                    .then(assembler => { assembler.setTotalSize(download.size); return assembler; });
            }
            ...
            return download.assemblerProm.then(assembler => {
                ...
                const worker = new DownloadWorker(job, this.makeProgressCB(job, download), (pause, replaceFileName) => replaceFileName !== undefined
                ...
                job.state = 'paused';
                if (err.code === 'EBUSY') {
                    return Promise.reject(new CustomErrors_1.ProcessCanceled('output file is locked'));
                }
                else {
        ...
        this.makeDataCB = (download) => {
            ...
            return (offset, data) => {
                ...
                if (download.assembler.isClosed()) {
                    return Promise.reject(new CustomErrors_1.ProcessCanceled('file already closed'));
                }
        ...
        this.finishChunk = (download, job, paused) => {
            ...
            if (!paused && hasRemainingData) {
                job.restartCount = ((_a = job.restartCount) !== null && _a !== void 0 ? _a : 0) + 1;
                if (job.restartCount > MAX_INTERNAL_AUTO_RESUMES) {
                    (0, log_1.log)('warn', 'auto-resume cap reached for chunk; finalizing with unfinishedChunks', {
                        downloadId: download.id,
                        workerId: job.workerId,
                    ...
                    const hasOtherActive = download.chunks.some((chunk) => (chunk !== job) && !['paused', 'finished'].includes(chunk.state));
                    (0, log_1.log)('info', 'auto-resuming current chunk internally', {
                        downloadId: download.id,
                        workerId: job.workerId,
