...
const fsFast = __importStar(require("fs-extra"));
...
class FileAssembler {
    ...
    rename(newName) {
        const closeFD = () => this.isClosed()
            ? Promise.reject(new CustomErrors_1.ProcessCanceled('File is closed'))
    ...
    addChunk(offset, data) {
        ...
        return this.mQueue(() => ((this.mFD === undefined)
            ? Promise.reject(new CustomErrors_1.ProcessCanceled('file already closed'))
            ...
            .catch(err => {
            if (err.code === 'ENOSPC') {
                return (dialog.showMessageBoxSync((0, errorHandling_1.getVisibleWindow)(), {
                    type: 'warning',
                    title: 'Disk is full',
