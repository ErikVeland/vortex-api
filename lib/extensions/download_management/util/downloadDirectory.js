...
function writeDownloadsTag(api, tagPath) {
    ...
    return writeTag().catch((err) => {
        if ((err === null || err === void 0 ? void 0 : err.code) !== 'EISDIR') {
            // Not the “directory instead of file” case → rethrow
            throw err;
        }
        // __vortex_downloads_folder exists as a directory; ask to re-init
        return api
            .showDialog('question', 'Reinitialize Tag', {
            ...
            : Promise.reject(err))
            .catch((inner) => {
            // If the directory disappeared in the meantime, continue; else rethrow the original error
            if ((inner === null || inner === void 0 ? void 0 : inner.code) === 'ENOENT') {
                return;
            }
            throw err;
        })
...
function removeDownloadsMetadata(api) {
    ...
    return (0, promise_helpers_1.promiseEach)(Object.keys(downloads), dlId => {
    ...
    return fs
        ...
        if (tag.instance !== state.app.instanceId) {
            return api
                ...
                .then(result => result.action === 'Cancel' ? Promise.reject(new CustomErrors_1.UserCanceled()) : Promise.resolve());
        }
        return;
    })
        .catch(() => {
...
function ensureDownloadsDirectory(api) {
    ...
    return fs
        ...
        .catch((err) => {
        var _a;
        if (!dirExists && Object.keys((_a = state.persistent.downloads.files) !== null && _a !== void 0 ? _a : {}).length === 0) {
            ...
            return fs
                .ensureDirWritableAsync(currentDownloadPath, () => Promise.resolve())
                .catch((inner) => {
                if ((inner === null || inner === void 0 ? void 0 : inner.code) === 'ENOENT') {
        ...
        return queryDownloadFolderInvalid(api, err, dirExists, currentDownloadPath).then(result => {
            if (result.action === 'Quit Vortex') {
                ...
                return removeDownloadsMetadata(api)
                    ...
                    return Promise.reject(new CustomErrors_1.ProcessCanceled('Failed to reinitialize download directory'));
                })
                    .finally(() => {
                    api.dismissNotification(id);
                });
                ...
                return api
                    ...
                    return validateDownloadsTag(api, path.join(selectedPath, exports.DOWNLOADS_DIR_TAG)).then(() => {
