...
class GameModeManager {
    constructor(api, extensionGames, gameStubs, gameStoreExtensions, onGameModeActivated) {
        this.mActiveSearchCancelled = false;
        this.storeGame = (game) => {
            return {
                name: game.name,
        ...
        this.onDiscoveredGame = (gameId, result) => {
            if (result === undefined) {
                const currentProfile = (0, activeGameId_1.activeProfile)(this.mStore.getState());
                ...
                (0, log_1.log)('info', 'Game discovered and added to store', {
                    gameId,
                    path: result.path,
                    store: result.store
                });
    ...
    refreshKnownGames() {
        if (this.mStore !== null) {
            const gamesStored = this.mKnownGames
                .map(this.storeGame)
                .filter(this.isValidGame);
            this.mStore.dispatch((0, session_1.setKnownGames)(gamesStored));
        }
    }
    /**
    ...
    setGameMode(oldMode, newMode, profileId) {
        ...
        return (0, discovery_2.assertToolDir)(game, gameDiscovery.path)
            ...
            const currentProfile = (0, activeGameId_1.activeProfile)(state);
            ...
            return ['ENOENT', 'ENOTFOUND'].includes(err.code)
                ? Promise.reject(new CustomErrors_1.SetupError('Missing: ' + (err.filename || modPath)))
                : Promise.reject(err);
        });
    }
    /**
    ...
    setupGameMode(gameMode) {
        ...
        if ((gameDiscovery === null || gameDiscovery === void 0 ? void 0 : gameDiscovery.path) === undefined) {
            ...
            // situation.
            return Promise.reject(new CustomErrors_1.ProcessCanceled('game not discovered'));
        }
            ...
            try {
                return (0, discovery_2.assertToolDir)(game, gameDiscovery.path)
                    ...
                    ? Promise.reject(new CustomErrors_1.ProcessCanceled(`Game folder "${gameDiscovery.path}" doesn't exist (any more).`))
                    : Promise.reject(err));
            }
            catch (err) {
                return Promise.reject(err);
     ...
     * @memberOf GameModeManager
     */
    startQuickDiscovery(games, showProgress = true) {
        var _a, _b;
        // Prevent overlapping quick discovery runs
        const state = this.mStore.getState();
        if ((_b = (_a = state.session) === null || _a === void 0 ? void 0 : _a.discovery) === null || _b === void 0 ? void 0 : _b.running) {
            (0, log_1.log)('info', 'Quick discovery already running, skipping duplicate trigger');
            return Promise.resolve([]);
        }
        const gamesToDiscover = games !== null && games !== void 0 ? games : this.mKnownGames;
        // Set up progress tracking only if showProgress is true
        if (showProgress) {
            this.mStore.dispatch((0, discovery_1.setPhaseCount)(gamesToDiscover.length));
        }
        // Progress callback for macOS discovery - only dispatch if showProgress is true
        const progressCallback = showProgress ? (gameId, step, percent) => {
            const gameIndex = gamesToDiscover.findIndex(g => g.id === gameId);
            if (gameIndex !== -1) {
                this.mStore.dispatch((0, discovery_1.discoveryProgress)(gameIndex, percent, `${step} - ${gameId}`));
            }
        } : undefined;
        return this.reloadStoreGames()
            .then(() => (0, discovery_2.quickDiscovery)(gamesToDiscover, this.mStore.getState().settings.gameMode.discovered, this.onDiscoveredGame, this.onDiscoveredTool, progressCallback))
            .then(() => {
            if (showProgress) {
    ...
    startToolDiscovery(gameId) {
        ...
        if (game !== undefined) {
            ...
            const state = this.mStore.getState();
            const installedExtensions = ((_a = state.session.extensions) === null || _a === void 0 ? void 0 : _a.installed) || {};
            ...
            const gameExtension = Object.values(installedExtensions).find(ext => ext.type === 'game' && ext.path &&
            ...
            if (gameExtension) {
                ...
                (0, log_1.log)('info', 'Attempting to load missing game extension', { gameId, extensionPath: gameExtension.path });
                try {
                    ...
                    if (typeof extensionModule.default === 'function') {
                        ...
                        const contextProxy = new Proxy({
                            ...
                            registerGame: (game) => {
                                ...
                                (0, log_1.log)('info', 'Dynamically registered game', { gameId: game.id });
                                this.mKnownGames.push(game);
                            },
                            registerGameStub: (game, extInfo) => {
                                // Handle game stubs if needed
                                (0, log_1.log)('info', 'Dynamically registered game stub', { gameId: game.id });
                            }
                        }, {
                            get: (target, prop) => {
                        ...
                        (0, log_1.log)('debug', 'Loading game extension dynamically', { extension: gameExtension.name, path: indexPath });
                        ...
                        if (newlyRegisteredGame) {
                            ...
                            return (0, discovery_2.quickDiscoveryTools)(gameId, newlyRegisteredGame.supportedTools, this.onDiscoveredTool)
                                .then(() => (0, api_1.getNormalizeFunc)(discovery.path))
                                .then(normalize => (0, discovery_2.discoverRelativeTools)(newlyRegisteredGame, discovery.path, discoveredGames, this.onDiscoveredTool, normalize));
                        }
                    }
                }
                catch (err) {
                    (0, log_1.log)('warn', 'Failed to dynamically load game extension', { gameId, error: err.message, stack: err.stack });
                }
            }
            // Log detailed information about what games are known
            (0, log_1.log)('warn', 'Game not found in known games list', {
    ...
    postDiscovery() {
        ...
        (0, promise_helpers_1.promiseMap)(Object.keys(discovered), gameId => {
            ...
            return (0, api_1.getNormalizeFunc)(discovered[gameId].path)
                ...
                (0, log_1.log)('error', 'failed to check if game should be overridden', { gameId, error: err.message });
                return Promise.resolve();
            });
        });
    }
