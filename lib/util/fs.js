...
function errorHandler(error, stackErr, tries, showDialogCallback, options) {
    ...
    return repProm
        .then(repeat => repeat
        ? Promise.resolve()
        : Promise.reject((0, util_1.restackErr)(error, stackErr)))
        .catch(err => Promise.reject((0, util_1.restackErr)(err, stackErr)));
}
function genFSWrapperAsync(func) {
    const wrapper = (stackErr, tries, ...args) => simfail(() => Promise.resolve(func(...args)))
...
const statSilentAsync = (statPath) => Promise.resolve(fs.stat(statPath));
...
function ensureDir(targetDir, onDirCreatedCB) {
    ...
    //  we create.
    const created = [];
    const mkdirRecursive = (dir) => Promise.resolve(fs.mkdir(dir))
        .then(() => {
        created.push(dir);
...
function selfCopyCheck(src, dest) {
    return Promise.all([fs.stat(src, { bigint: true }),
        ...
        ? Promise.reject(new Error(`Source "${src}" and destination "${dest}" are the same file (id "${stats[0].ino}").`))
        : Promise.resolve());
}
...
function moveRenameAsync(src, dest) {
    return moveAsync(src, dest, { overwrite: false })
        ...
        return moveRenameAsync(src, nextName(dest));
    }
    else {
        return Promise.reject(err);
    } });
    ...
    return simfail(() => Promise.resolve(fs.rename(sourcePath, destinationPath)))
        ...
        return (err.code === 'EPERM')
            ? Promise.resolve(fs.stat(destinationPath))
                ...
                ? Promise.reject((0, util_1.restackErr)(err, stackErr))
                : errorHandler(err, stackErr, tries)
                    .then(() => renameInt(sourcePath, destinationPath, stackErr, tries - 1)))
                .catch(newErr => Promise.reject((0, util_1.restackErr)(newErr, stackErr)))
            : errorHandler(err, stackErr, tries)
                .then(() => renameInt(sourcePath, destinationPath, stackErr, tries - 1));
    });
}
        ...
        if (['EPERM', 'EBADF', 'UNKNOWN', 'EEXIST', 'EROFS'].indexOf(err.code) !== -1) {
            return Promise.resolve(confirm())
                ...
                return elevated((ipcPath, req) => {
                    ...
                    return Promise.reject((0, util_1.restackErr)(err, stackErr));
                });
            });
        }
        else {
...
function changeFileOwnership(filePath, stat) {
    ...
    if ((stat.mode & readAndWriteOther) === readAndWriteOther) {
        return Promise.reject(new CustomErrors_1.ProcessCanceled('Ownership change not required'));
    }
...
function changeFileAttributes(filePath, wantedAttributes, stat) {
    return this.changeFileOwnership(filePath, stat)
        ...
        .catch(err => { if (err instanceof CustomErrors_1.ProcessCanceled) {
        return Promise.resolve();
    }
    else {
        return Promise.reject(err);
