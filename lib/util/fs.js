...
exports.readlinkAsync = exports.removeAsync = exports.rmdirAsync = exports.renameAsync = exports.unlinkAsync = exports.removeSync = exports.linkAsync = exports.copyAsync = exports.moveRenameAsync = exports.ensureDirAsync = exports.ensureFileAsync = exports.ensureDirSync = exports.isDirectoryAsync = exports.writeFileAsync = exports.writeAsync = exports.utimesAsync = exports.symlinkAsync = exports.statSilentAsync = exports.statAsync = exports.readFileAsync = exports.readAsync = exports.readdirAsync = exports.openAsync = exports.moveAsync = exports.mkdirsAsync = exports.mkdirAsync = exports.lstatAsync = exports.fsyncAsync = exports.closeAsync = exports.chmodAsync = exports.appendFileAsync = exports.genFSWrapperAsync = exports.setTFunction = exports.writeSync = exports.writeFileSync = exports.watch = exports.symlinkSync = exports.statSync = exports.readFileSync = exports.readdirSync = exports.openSync = exports.linkSync = exports.createWriteStream = exports.createReadStream = exports.closeSync = exports.appendFileSync = exports.accessSync = exports.WriteStream = exports.Stats = exports.constants = void 0;
exports.readFileBOM = exports.encodingFromBOM = exports.withTmpFile = exports.withTmpDir = exports.withTmpDirImpl = exports.forcePerm = exports.makeFileWritableAsync = exports.changeFileAttributes = exports.changeFileOwnership = exports.ensureDirWritableAsync = void 0;
...
function unlockConfirm(filePath) {
    ...
    try {
        ...
        (0, log_1.log)('warn', 'failed to determine list of processes locking file', { filePath, error: err.message });
    }
...
function unknownErrorRetry(filePath, err, stackErr) {
    ...
    switch (options.buttons[choice]) {
        case 'Retry': return bluebird_1.default.resolve(true);
        case 'Ignore': {
...
function errorRepeat(error, filePath, retries, stackErr, showDialogCallback, options) {
    if ((retries > 0)
        ...
        return bluebird_1.default.delay(retries === 1 ? 1000 : 100)
...
function genFSWrapperAsync(func) {
    const wrapper = (stackErr, tries, ...args) => simfail(() => bluebird_1.default.resolve(func(...args)))
...
function isDirectoryAsync(dirPath) {
    return bluebird_1.default.resolve(fs.stat(dirPath))
...
function ensureFileAsync(filePath) {
    ...
    return bluebird_1.default.resolve(fs.ensureFile(filePath))
        .catch(err => {
...
function selfCopyCheck(src, dest) {
    return bluebird_1.default.all([fs.stat(src, { bigint: true }),
        fs.stat(dest, { bigint: true })
            .catch(err => {
        ...
        ? bluebird_1.default.reject(new Error(`Source "${src}" and destination "${dest}" are the same file (id "${stats[0].ino}").`))
        : bluebird_1.default.resolve());
}
...
function copyFileCloneFallback(src, dest, options) {
    if (process.platform !== 'darwin') {
        return bluebird_1.default.resolve(fs.copy(src, dest, options));
    }
    return bluebird_1.default.resolve(fs.stat(src))
        .then(stat => {
        if (!stat.isFile()) {
            return bluebird_1.default.resolve(fs.copy(src, dest, options));
        }
        const overwrite = ((options === null || options === void 0 ? void 0 : options.overwrite) !== undefined) ? options.overwrite : true;
        const ensureNotExists = overwrite
            ? bluebird_1.default.resolve()
            : bluebird_1.default.resolve(fs.pathExists(dest))
                .then(exists => {
                if (exists) {
        ...
        return ensureNotExists
            .then(() => {
            var _a, _b;
            return nodeFS.promises.copyFile(src, dest, ((_b = (_a = nodeFS.constants) === null || _a === void 0 ? void 0 : _a.COPYFILE_FICLONE) !== null && _b !== void 0 ? _b : 0));
        })
            .then(() => undefined)
            .catch((err) => {
...
function copyInt(src, dest, options, stackErr, tries) {
    return simfail(() => (process.platform === 'darwin'
        ...
        : bluebird_1.default.resolve(fs.copy(src, dest, options))))
        .catch((err) => errorHandler(err, stackErr, tries, options === null || options === void 0 ? void 0 : options.showDialogCallback, { extraRetryErrors: ['EEXIST'] })
...
function linkInt(src, dest, stackErr, tries, options) {
    return simfail(() => bluebird_1.default.resolve(fs.link(src, dest)))
        .catch((err) => errorHandler(err, stackErr, tries, (options !== undefined) ? options.showDialogCallback : undefined)
...
function unlinkInt(filePath, stackErr, tries, options) {
    return simfail(() => bluebird_1.default.resolve(fs.unlink(filePath)))
        .catch((err) => {
...
function renameInt(sourcePath, destinationPath, stackErr, tries) {
    return simfail(() => bluebird_1.default.resolve(fs.rename(sourcePath, destinationPath)))
        .catch((err) => {
        if ((tries > 0) && RETRY_ERRORS.has(err.code)) {
            return bluebird_1.default.delay((NUM_RETRIES - tries + 1) * RETRY_DELAY_MS)
...
function rmdirInt(dirPath, stackErr, tries) {
    return simfail(() => bluebird_1.default.resolve(fs.rmdir(dirPath)))
        .catch((err) => {
        if (err.code === 'ENOENT') {
            // don't mind if a file we wanted deleted was already gone
            return bluebird_1.default.resolve();
            ...
            return bluebird_1.default.delay(RETRY_DELAY_MS)
...
function removeInt(remPath, stackErr, tries, options) {
    return simfail(() => rimrafAsync(remPath))
        .catch(err => errorHandler(err, stackErr, tries, options.showDialogCallback, { enotempty: true })
...
function readlinkInt(linkPath, stackErr, tries) {
    return simfail(() => bluebird_1.default.resolve(fs.readlink(linkPath)))
        .catch(err => {
        if ((err.code === 'UNKNOWN') && (0, platform_1.isWindows)()) {
...
function elevated(func, parameters) {
    ...
    return new bluebird_1.default((resolve, reject) => {
        ...
        vortexRun.runElevated(ipcPath, func, parameters)
            .catch(err => {
            if ((err.code === 5)
                || ((0, platform_1.isWindows)() && (err.systemCode === 1223))) {
...
function ensureDirWritableAsync(dirPath, confirm) {
    ...
    return bluebird_1.default.resolve(fs.ensureDir(dirPath))
        ...
        if (['EPERM', 'EBADF', 'UNKNOWN', 'EEXIST', 'EROFS'].indexOf(err.code) !== -1) {
            return bluebird_1.default.resolve(confirm())
                .then(() => {
                const userId = permission.getUserId();
                return elevated((ipcPath, req) => {
...
function changeFileOwnership(filePath, stat) {
    ...
    return (stat.uid !== process.getuid())
        ? (!hasGroupPermissions) || (hasGroupPermissions && (stat.gid !== process.getgid()))
            ? bluebird_1.default.resolve(fs.chown(filePath, process.getuid(), stat.gid))
...
function makeFileWritableAsync(filePath) {
    const stackErr = new Error();
    const wantedAttributes = (0, platform_1.isWindows)() ? parseInt('0666', 8) : parseInt('0600', 8);
...
function forcePerm(t, op, filePath, maxTries = 3) {
    return op()
        ...
        if ((['EPERM', 'EACCES'].indexOf(err.code) !== -1) || (err.systemCode === 5)) {
            const wantedAttributes = (0, platform_1.isWindows)()
                ? parseInt('0666', 8)
                : parseInt('0600', 8);
            ...
            if (err !== null) {
                ...
                cb(tmpPath)
                    ...
                    try {
                        ...
                        // cleanup failed
                        (0, log_1.log)('warn', 'Failed to clean up temporary directory', { tmpPath });
                    }
                });
            }
        });
