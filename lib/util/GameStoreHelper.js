...
class GameStoreHelper {
    ...
    isGameInstalled(id, storeId) {
        return ((storeId !== undefined)
            ...
            .then(entry => Promise.resolve(entry === null || entry === void 0 ? void 0 : entry.gameStoreId))
    ...
    registryLookup(lookup) {
        ...
        try {
            const instPath = (_a = winapi === null || winapi === void 0 ? void 0 : winapi.RegGetValue) === null || _a === void 0 ? void 0 : _a.call(winapi, chunked[0], chunked[1], chunked[2]);
            ...
            return Promise.reject(new IGameStore_1.GameEntryNotFound(lookup, 'registry'));
        }
    }
    findByName(name, storeId) {
        return this.validInput(name)
            ? this.findGameEntry('name', name, storeId)
            : Promise.reject(new IGameStore_1.GameEntryNotFound('Invalid name input', this.mStores.map(store => store.id).join(', ')));
    }
    findByAppId(appId, storeId) {
        return this.validInput(appId)
            ? this.findGameEntry('id', appId, storeId)
            : Promise.reject(new IGameStore_1.GameEntryNotFound('Invalid appId input', this.mStores.map(store => store.id).join(', ')));
    }
    ...
    reloadGames(api) {
        ...
        (0, log_1.log)('info', 'reloading game store games', stores.map(store => store.id)
            .join(', '));
        return (0, promise_helpers_1.promiseEach)(stores, (store) => ((store === null || store === void 0 ? void 0 : store.reloadGames) !== undefined)
            ? store.reloadGames()
                .catch(err => {
    ...
    isStoreRunning(storeExecPath) {
        const runningProcesses = winapi === null || winapi === void 0 ? void 0 : winapi.GetProcessList();
    ...
    findGameEntry(searchType, pattern, storeId) {
        ...
        if (!!storeId) {
            try {
                ...
                return Promise.reject(new IGameStore_1.GameEntryNotFound(name, stores));
            }
        }
        ...
        return (0, promise_helpers_1.promiseReduce)(gameStores, (accum, store) => store.allGames()
            .then(entries => {
            ...
            .catch(err => { if (err instanceof IGameStore_1.GameEntryNotFound) {
            return Promise.resolve(accum);
        }
        else {
