...
function isCaseSensitiveFailed(testPath, reason) {
    if (testPath === undefined) {
        return Promise.resolve(!(0, platform_1.isWindows)());
    }
...
function isCaseSensitive(testPath) {
    return Promise.resolve(fsOrig.readdir(testPath))
        ...
        return (0, promise_helpers_1.promiseMap)([fileName, fileName.toLowerCase(), fileName.toUpperCase()], file => Promise.resolve(fsOrig.stat(path.join(testPath, file))).then(value => ({ status: "fulfilled", value })).catch(err => ({ status: "rejected", reason: err })));
    })
        .then((stats) => {
        if (stats === null) {
...
function getNormalizeFunc(testPath, parameters) {
    ...
    return isCaseSensitive(testPath)
        ...
        let funcOut = caseSensitive
            ? (input) => input
            : genNormalizeCase();
        if ((parameters['separators'] !== false) && (0, platform_1.isWindows)()) {
