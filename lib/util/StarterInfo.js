...
class StarterInfo {
        ...
        return launcherPromise.then(res => {
            if (res !== undefined) {
                ...
                return StarterInfo.runThroughLauncher(res.launcher, info, api, infoObj)
                    ...
                    .catch(err => {
                    if (err instanceof IGameStore_1.GameStoreNotFound) {
    ...
    static runDirectly(info, api, onShowError, onSpawned) {
        ...
        return api.runExecutable(info.exePath, info.commandLine, {
            ...
            .catch(err => {
            if (err instanceof CustomErrors_1.MissingDependency) {
    ...
    initFromGame(game, gameDiscovery) {
        ...
        const executablePath = (0, macOSGameCompatibility_1.getExecutablePathForPlatform)(gameDiscovery.path, this.gameId, executable);
