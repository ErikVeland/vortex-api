...
const ErrorBoundary_1 = require("../../controls/ErrorBoundary");
...
class LinkTag extends bbcode_to_react_1.Tag {
    ...
    toHTML() {
        ...
        return this.renderer.context({ linkify: false }, () => [`<a href="${linkUrl}" target="_blank" title="${linkUrl}">`, this.getContent(), '</a>']);
    }
    toReact() {
        let linkUrl = this.renderer.strip(this.params[this.name] || this.getContent(true));
        if (/javascript:/i.test(linkUrl)) {
