...
function extractWithTar(archivePath, destPath) {
    ...
    const proc = (0, child_process_1.spawn)('tar', [...args, archivePath, '-C', destPath, ...exclude, ...xattrs]);
...
function extractWithBSDTar(archivePath, destPath) {
    ...
    const proc = (0, child_process_1.spawn)(bsdtarPath, args);
    return new Promise((resolve, reject) => {
        let stderr = '';
        proc.stderr.on('data', (d) => { stderr += d.toString(); });
        proc.on('error', err => reject(err));
...
function extractWithDittoZip(archivePath, destPath) {
    ...
    const proc = (0, child_process_1.spawn)(dittoPath, ['-x', '-k', archivePath, destPath]);
    return new Promise((resolve, reject) => {
        let stderr = '';
        proc.stderr.on('data', (d) => { stderr += d.toString(); });
        proc.on('error', err => reject(err));
...
function extractWithUnzip(archivePath, destPath) {
    ...
    const proc = (0, child_process_1.spawn)(unzipPath, ['-o', '-qq', '-X', archivePath, '-d', destPath]);
    return new Promise((resolve, reject) => {
        let stderr = '';
        proc.stderr.on('data', (d) => { stderr += d.toString(); });
        proc.on('error', err => reject(err));
...
function resolveToolPathSync(cmd) {
    try {
        const res = (0, child_process_1.spawnSync)('which', [cmd], { encoding: 'utf8' });
        if (res && res.status === 0) {
...
function extractWithUnar(archivePath, destPath) {
    ...
    const proc = (0, child_process_1.spawn)(unarPath, args);
    return new Promise((resolve, reject) => {
        let stderr = '';
        proc.stderr.on('data', (d) => { stderr += d.toString(); });
        proc.on('error', err => reject(err));
...
function toolAvailableSync(cmd, args = ['--version']) {
    ...
    try {
        const res = (0, child_process_1.spawnSync)(resolved, args, { stdio: 'ignore' });
